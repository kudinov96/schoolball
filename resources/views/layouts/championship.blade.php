<!doctype html>
<html lang="en">
<head>

    <script src="https://www.googleoptimize.com/optimize.js?id=OPT-W4WHQTK"></script>


    <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
                new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
            j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
            'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
        })(window,document,'script','dataLayer','GTM-MT69BWD');</script>
    <!-- End Google Tag Manager -->


    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <meta name="csrf-token" content="{{ csrf_token() }}" />
    @yield('description')

    <!-- Bootstrap CSS -->
    <link rel="shortcut icon" href="{{ asset('assets/favicon-32x32.png') }}" type="image/png">
    <link rel="stylesheet" href="{{ asset('css/bootstrap-4.5.0-dist/css/bootstrap.min.css') }}">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Sans&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="{{ asset('css/style_event.css') }}">

    <link rel="stylesheet" href="{{ asset('css/font-awesome.min.css') }}">
    <link rel="stylesheet" href="{{ asset('css/datepicker.min.css') }}">
    <link href="{{ asset('css/toast.css') }}" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="{{ asset('js/slick/slick.css') }}">
    <link rel="stylesheet" type="text/css" href="{{ asset('js/slick/slick-theme.css') }}">
    <link rel="stylesheet" type="text/css" href="{{ asset('css/flipper.css') }}">
    <link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.css" />



    <link rel="stylesheet" type="text/css" href="{{ asset('js/owlcarousel/assets/owl.carousel.min.css') }}">

    <link rel="stylesheet" type="text/css" href="{{ asset('js/owlcarousel/assets/owl.theme.default.css') }}">

    <link rel="stylesheet" href="{{ asset('css/floating_forms.css') }}">

    <link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/1.10.21/css/dataTables.bootstrap4.min.css"/>


    @yield('title')


    <script type="text/javascript" src="https://cdn.jsdelivr.net/jquery/latest/jquery.min.js"></script>
    <script type="text/javascript" src="https://cdn.jsdelivr.net/momentjs/latest/moment.min.js"></script>
    <script type="text/javascript" src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>

    <script src=" {{ asset('js/owlcarousel/owl.carousel.js') }}"></script>





    <script src="{{ asset('js/toast.js') }}" ></script>
    <script src="https://www.google.com/recaptcha/api.js" async defer></script>
    <script>
        (function(w, d, u, i, o, s, p) {
            if (d.getElementById(i)) { return; } w['MangoObject'] = o;
            w[o] = w[o]  function() { (w[o].q = w[o].q  []).push(arguments) }; w[o].u = u; w[o].t = 1 * new Date();
            s = d.createElement('script'); s.async = 1; s.id = i; s.src = u;
            p = d.getElementsByTagName('script')[0]; p.parentNode.insertBefore(s, p);
        }(window, document, '//widgets.mango-office.ru/widgets/mango.js', 'mango-js', 'mgo'));
        mgo({calltracking: {id: 30114, elements: [{"numberText":"74951206006"}], domain: 'schoolball.ru'}});
    </script>

 </head>
<body>

<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-MT69BWD"
                  height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>

<div class="d-sm-block d-md-block d-lg-none" style="overflow:hidden;width:100%;">
    <div class=" mobile-header "  >




            <div  class="d-none d-sm-block" style="background-color: #f7f7f9;">

                <div class="container-fluid" >
                    <div class="topHeader">
                        <div class="row">
                            <div class="col-md-4  tabletHeader">
                                <span class="tablet-header">Детская футбольная школа в Москве и МО </span>
                            </div>
                            <div class="col-md-2 tabletHeader">
                                <img style="width: 16px; height: 16px;"  src="{{ asset('assets/sb-ui/icons/star.svg')  }}"  > <span class="tablet-header raiting"> 4.0 </span>
                                <span id="reviews" class="tablet-header rev"> 576 отзывов </span>

                            </div>

                            <div class="col-md-4 offset-2 tabletHeader">
                    <span class="tablet-header phone"> +7 495 120-60-06 </span>
                                <span id="reviews" class="tablet-header gray"> Ежедневно с 10 до 20 </span>
                                <img class="lk-btn" style="width: 16px; height: 16px;"  src="{{ asset('assets/sb-ui/icons/profile-circle.svg')  }}"  >

                            </div>


                        </div>
                    </div>
                </div>
        </div>


        <div class="bottomOfHeader">
            <div class="row">
                <div class="col-5 col-md-3 text-center mobileHeaderlogo">
                    <a href="#">
                        <img   class="logo_lager" src="{{ asset('assets/sb-ui/logo-white.svg')}}"  >                    </a>
                </div>
                <div class="col-5 col-md-3 offset-md-5 text-center d-none d-sm-block  ">
                    <div class="yellowButton text-center   "  data-toggle="modal" data-animation="bounce" data-target=".bs-example-modal-lg">
                        Записаться на тренировку
                    </div>

                </div>
            <div class="col-5 text-center  d-sm-none" style="
    padding-right: 15px;
    padding-left: 10px;
">
            <div class="yellowButton text-center   "  data-toggle="modal" data-animation="bounce" data-target=".bs-example-modal-lg">
                Записаться
            </div>
        </div>
                <div class="col-1 menuButton" style="    padding: 0px;">
                    <img   class="mobMenuBut" style="    margin: 5px 0px 0px -7px !important;"  id="mobMenuTog"  src="{{ asset('assets/sb-ui/icons/menu.svg')}}"  >

                </div>
            </div>
        </div>
    </div>
    <div id="mobMenu" class="mobMenu d-none">

        <div class="mobMenuButtons">
            <a onclick="showMenuContent()" href="#"><i class="fas fa-close" id="mobMenuTog" style="font-size: 11px;margin: 7px 5px 0px 0px;color: #9e9e9efc;" aria-hidden="true"></i> ЗАКРЫТЬ</a>

            <a onclick="showMenuContent()" href="/home">ГЛАВНАЯ</a>
            <a onclick="showMenuContent()"  href="/home#methodsmob">МЕТОДИКА</a>
            <a onclick="showMenuContent()"  href="/home#coachesmob">ТРЕНЕРЫ</a>
            <a onclick="showMenuContent()"  href="/clubs">КЛУБЫ</a>
            <a onclick="showMenuContent()"  href="/camp/cao">ЛАГЕРЯ</a>

            <a onclick="showMenuContent()" href="/home#reviewsmob">ОТЗЫВЫ</a>
            <a onclick="showMenuContent()" href="/blog">БЛОГ</a>

        </div>
        <div class="mobMenuLC">
            @guest
                <a href="https://sportlead.ru/sball/lk/" target="_blank"  style="color: #2f3236;;"><img src="{{ asset('assets/bxs-user.svg') }}" alt=""> <u>Мой кабинет</u></a>
            @else

                <a href="{{ route('clients') }}" style="color: #2f3236;;"><img src="{{ asset('assets/bxs-user.svg') }}" alt=""> <u>Мой кабинет ( {{ Auth::user()->name }}
                        {{ Auth::user()->name }}
                        )</u> </a>

            @endguest

        </div>
    </div>
</div>
<style>
    .mobile-header{
        position: fixed;
        top: 0px;
        z-index: 100;
        width: 100%;
    }
    .header {

        height: 50px;
        z-index: 3;
        position: fixed;
        left: 0;
        top: 0;
        width: 100%;
        transition: transform .1s linear;
    }

    .header.hide {
        transform: translateY(-50px);
    }



    .header-mob {

        height: 50px;
        z-index: 3;

        left: 0;
        top: 0;
        width: 100%;
        transition: transform .2s linear;
    }

    .header-mob.hide {
        transform: translateY(-50px);
    }

    .mobile-header.hide {
        transform: translateY(-50px);
        transition: transform .1s linear;
    }
    @media (min-width: 0px) and (max-width: 480px)
    {
        .overflowHidden {
            overflow: hidden;
            opacity: 0;
            transition: opacity 2s ease-in-out;
        }

        .overflowHidden:hover {
            opacity: 1;
        }
    }


</style>
<div id="app" class="header d-none d-md-block">

    <div class="container-fluid" style="    top: 0px;
    height: 148px;
    z-index: 100;
    background: #f7f7f9;
    /* border-bottom-right-radius: 2000px 200px; */
    /* border-bottom-left-radius: 2000px 200px;">
        <div class="topHeader">
            <div class="row">
                <div class="col-md-3 text-center topHeaderLocation">
                    <h4>Детская футбольная школа в Москве и МО </h4>
                </div>
                <div class="col-md-3 text-center topHeaderYandexRatio">
                    <div class="row">

                        <div class="col-md-1 yandexStars" style="margin-top: 6px;">
                            <span style="font-size: 13px;color:#fec311; margin-top: -3px;">4.7 </span>
                        </div>
                        <div class="col-md-3">
                            <img src="{{ asset('assets/Group 19.png') }}" alt="" style="margin-top: 10px;width: 75px; ">
                        </div>
                        <div class="col-md-7 text-right topHeaderYandexReview">
                            <p><u>576 отзывов на Яндекс</u></p>
                        </div>

                    </div>
                </div>
                <div class="col-md-4 text-center topHeaderTelephone">
                    <h4><a href="tel:+74951206006" style=" color: #000;">+7 (495) 120-60-06 </a> <span style="margin-left: 22px; color: #6e6d7a;"> Ежедневно с 10:00 до 20:00 </span></h4>
                </div>

                <div class="col-md-2 text-center topHeaderLK">
                    @guest
                        <h4><a href="https://sportlead.ru/sball/lk/" style="color: #303337;"><img src="{{ asset('assets/bxs-user.svg') }}" alt="" >  <u>Мой кабинет</u> <div class="Line-Copy-2"></div>  </a></h4>
                    @else
                        <h4><a href="{{ route('clients') }}" style="color: #303337;"><img src="{{ asset('assets/bxs-user.svg') }}" alt="" >  <u>Мой кабинет</u> (                                    {{ Auth::user()->name }}
                                {{ Auth::user()->name }}
                                ) <div class="Line-Copy-2"></div>  </a></h4>


                    @endguest

                </div>
            </div>
        </div>
        <div class="mainHeader">
            <div class="row">
                <!-- <div class="col-md-9"> -->
                <!-- <div class="row"> -->

                <?php
                /*
                                      <div class="col-md-1 mainHeaderLogo text-center">
                                        <a href="https://schoolball.ru/#">
                                        <?xml version="1.0" encoding="UTF-8"?>
                                        <svg width="80px" height="56px" viewBox="0 0 80 56" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                            <defs>
                                                <rect id="path-1" x="0" y="0" width="1216" height="72" rx="8"></rect>
                                                <filter x="-3.0%" y="-51.4%" width="106.1%" height="202.8%" filterUnits="objectBoundingBox" id="filter-2">
                                                    <feMorphology radius="1" operator="dilate" in="SourceAlpha" result="shadowSpreadOuter1"></feMorphology>
                                                    <feOffset dx="0" dy="0" in="shadowSpreadOuter1" result="shadowOffsetOuter1"></feOffset>
                                                    <feGaussianBlur stdDeviation="12" in="shadowOffsetOuter1" result="shadowBlurOuter1"></feGaussianBlur>
                                                    <feComposite in="shadowBlurOuter1" in2="SourceAlpha" operator="out" result="shadowBlurOuter1"></feComposite>
                                                    <feColorMatrix values="0 0 0 0 0.323977461   0 0 0 0 0.381127872   0 0 0 0 0.466853487  0 0 0 0.15 0" type="matrix" in="shadowBlurOuter1"></feColorMatrix>
                                                </filter>
                                            </defs>
                                            <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                <g id="Menu" transform="translate(-32.000000, -56.000000)">
                                                    <g id="Group-76">
                                                        <g id="Group-45" transform="translate(0.000000, 48.000000)">
                                                            <g id="Rectangle" opacity="0.600000024">
                                                                <use fill="black" fill-opacity="1" filter="url(#filter-2)" xlink:href="#path-1"></use>
                                                                <rect stroke="#FFFFFF" stroke-width="1" fill="#FFFFFF" fill-rule="evenodd" x="-0.5" y="-0.5" width="1217" height="73" rx="8"></rect>
                                                            </g>
                                                            <g id="SchoolBall_Logo_Rus-3" transform="translate(32.000000, 8.000000)">
                                                                <g id="SchoolBall_Logo_Rus-2">
                                                                    <g id="SchoolBall_Logo_Rus">
                                                                        <path d="M76.9129961,28.8800676 L3.06957198,28.8800676 C1.44607004,28.8800676 0.124980545,30.2190197 0.124980545,31.8651042 L0.124980545,44.6633915 C0.124980545,46.3086873 1.44607004,47.6477972 3.06957198,47.6477972 L76.9129961,47.6477972 C78.5368093,47.6477972 79.8578988,46.3086873 79.8578988,44.6633915 L79.8578988,31.8651042 C79.8578988,30.2190197 78.5368093,28.8800676 76.9129961,28.8800676" id="Fill-3" fill="#FFFFFE"></path>
                                                                        <path d="M39.991284,0.0900732394 C24.7950195,0.0900732394 12.4322179,12.6203493 12.4322179,28.0225577 C12.4322179,43.4238197 24.7950195,55.9540958 39.991284,55.9540958 C55.1875486,55.9540958 67.5506615,43.4238197 67.5506615,28.0225577 C67.5506615,12.6203493 55.1875486,0.0900732394 39.991284,0.0900732394" id="Fill-4" fill="#FFFFFE"></path>
                                                                        <path d="M25.4898054,13.8178028 C25.5634241,13.7376676 25.6406226,13.6594254 25.7184436,13.5807099 C28.6759533,10.5832113 32.4,8.62242254 36.4992996,7.89931268 L36.9148638,7.82580282 L36.6375097,8.14823662 C36.2084047,8.6484507 35.8720623,9.07468169 35.6381323,9.41494085 C35.5382101,9.56196056 35.4014008,9.76292958 35.3047471,9.97983099 L35.2729961,10.0516056 L35.1971984,10.0662761 C35.15393,10.0752676 30.8289494,10.9584901 26.9277821,14.281262 L26.8731518,14.3274817 L25.3067704,14.0173521 L25.4898054,13.8178028 Z M40.8779767,13.9766535 L40.9582879,14.0340732 L39.52607,20.9575662 L39.4552529,20.9891155 C35.5094163,22.7276394 33.0269261,24.3243493 32.1427237,24.9300958 L32.065214,24.9835718 L26.5909728,21.7887324 L26.5985992,21.6928225 C26.6641245,20.8805859 27.0138521,16.7534648 27.5021012,15.245093 L27.5150195,15.2053408 L27.5466148,15.1780507 C31.174786,11.9851042 35.6610117,11.1113465 35.7056809,11.1025127 L35.8387549,11.0952563 L35.8983658,11.1201803 C36.4037354,11.3626366 38.9827237,12.6198761 40.8779767,13.9766535 Z M42.0936965,14.1526986 C43.498677,13.629138 47.1842802,13.2753127 49.1402335,13.1486423 L49.1878599,13.1454873 L49.2284825,13.1710423 C52.8908949,15.4660958 54.8018677,18.817893 54.820856,18.8511775 L54.8470039,18.8980282 L54.8387549,18.9500845 C54.6731518,19.9968901 54.0747082,23.5651155 53.3077043,25.5514592 L53.2792218,25.6254423 L46.5797665,27.2590648 L40.5696498,21.168 L42.0158755,14.1809352 L42.0936965,14.1526986 Z M49.1047471,9.79021972 C51.0017121,10.7616225 52.7322957,12.0324282 54.2485603,13.5693521 C55.7802335,15.1211042 57.0459144,16.8962254 58.010428,18.8451831 L58.1794553,19.1867042 L55.7273152,18.2798197 L55.7000778,18.2336 C55.6790661,18.1979493 53.5637354,14.6451831 49.8311284,12.2783549 L49.789572,12.2523268 L48.7523735,9.60991549 L49.1047471,9.79021972 Z M20.5070817,33.2906592 C19.8263035,30.6968338 19.6544747,27.9804394 20.0152529,25.2739831 L20.0761089,24.821093 L21.0810895,26.6597859 L21.0761089,26.7078986 C20.825214,29.0188845 21.0954086,31.4554366 21.4205447,33.2906592 L22.5055253,33.2906592 C22.1715175,31.4715268 21.8835798,29.0272451 22.140856,26.7556958 L22.1598444,26.6970141 C22.8719066,25.455707 24.5634241,23.0145803 25.8692607,22.6612282 L25.9290272,22.6452958 L31.6233463,25.9668056 L33.1310506,33.2906592 L44.0555642,33.2906592 L46.7670039,28.3218028 L53.4561868,26.6924394 L53.5153307,26.7717859 C54.56,28.1758873 56.3905058,30.8705127 56.6205447,32.064969 L56.627393,32.099831 L56.6245914,32.1105577 C56.6227237,32.2326535 56.6040467,32.6416901 56.4490272,33.2906592 L57.5431907,33.2906592 C57.6038911,33.0007211 57.6396887,32.7588958 57.6602335,32.5711775 L57.6706615,32.4744789 L57.7612451,32.4435606 C58.2239689,32.2872338 58.9228016,31.9446085 59.6270817,31.2030423 L59.9596887,30.8533183 L59.8824903,31.3322366 C59.7758755,31.9955606 59.6347082,32.6481577 59.4672374,33.2906592 L60.5662257,33.2906592 C60.9433463,31.7643042 61.1626459,30.1882592 61.2032685,28.5819268 C61.2062257,28.5066817 61.2079377,28.4320676 61.2079377,28.3561915 C61.2521401,25.4850479 60.7453696,22.6855211 59.700856,20.0330141 L59.629572,19.8505014 C58.5657588,17.2115606 57.0084047,14.841893 55.0009339,12.8071211 C52.9953307,10.7747155 50.6578988,9.19677746 48.0521401,8.11747606 L47.9038132,8.05579718 C45.3845914,7.03454648 42.7256031,6.51587606 40.0015564,6.51587606 C39.4555642,6.51587606 38.9031907,6.53780282 38.3592218,6.58055211 L38.3511284,6.5811831 C38.3494163,6.5811831 38.3478599,6.5811831 38.3461479,6.5811831 L38.3142412,6.58402254 L38.3101946,6.58228732 C38.2518288,6.58528451 38.1943969,6.59049014 38.1397665,6.59711549 C33.1702724,7.03454648 28.4975875,9.23936901 24.9772763,12.8071211 C24.6073152,13.181769 24.2476265,13.5740845 23.9086381,13.9717634 C23.8775097,14.0086761 23.8627237,14.0257127 23.8488716,14.043538 C21.6714397,16.6212732 20.1157977,19.7254085 19.3450584,23.0257803 C19.3270039,23.095031 19.311751,23.1608113 19.2964981,23.2284845 L19.2930739,23.2504113 C18.5508171,26.5791775 18.6163424,30.0294085 19.4225681,33.2906592 L20.5070817,33.2906592 L20.5070817,33.2906592 Z" id="Fill-5" fill="#1A1919"></path>
                                                                        <path d="M39.991284,1.15233803 C25.3729183,1.15233803 13.4801556,13.206062 13.4801556,28.0225577 C13.4801556,42.8382648 25.3729183,54.8921465 39.991284,54.8921465 C54.6096498,54.8921465 66.5025681,42.8382648 66.5025681,28.0225577 C66.5025681,13.206062 54.6096498,1.15233803 39.991284,1.15233803 Z M39.991284,2.21444507 C54.0541634,2.21444507 65.4546304,13.7684282 65.4546304,28.0225577 C65.4546304,42.2751099 54.0541634,53.8297239 39.991284,53.8297239 C25.9284047,53.8297239 14.528249,42.2751099 14.528249,28.0225577 C14.528249,13.7684282 25.9284047,2.21444507 39.991284,2.21444507 L39.991284,2.21444507 Z" id="Fill-6" fill="#1A1919"></path>
                                                                        <path d="M3.06957198,46.0544 C2.31268482,46.0544 1.69696498,45.4301972 1.69696498,44.6633915 L1.69696498,31.8651042 C1.69696498,31.0975099 2.31268482,30.4734648 3.06957198,30.4734648 L76.9129961,30.4734648 C77.6700389,30.4734648 78.2859144,31.0975099 78.2859144,31.8651042 L78.2859144,44.6633915 C78.2859144,45.4301972 77.6700389,46.0544 76.9129961,46.0544 L3.06957198,46.0544" id="Fill-7" fill="#F0A936"></path>
                                                                        <path d="M76.9129961,29.9423324 L3.06957198,29.9423324 C2.02381323,29.9423324 1.17291829,30.8047324 1.17291829,31.8651042 L1.17291829,44.6633915 C1.17291829,45.7231324 2.02381323,46.5855324 3.06957198,46.5855324 L76.9129961,46.5855324 C77.9590661,46.5855324 78.8099611,45.7231324 78.8099611,44.6633915 L78.8099611,31.8651042 C78.8099611,30.8047324 77.9590661,29.9423324 76.9129961,29.9423324 Z M76.9129961,31.0044394 C77.3800778,31.0044394 77.7618677,31.3915493 77.7618677,31.8651042 L77.7618677,44.6633915 C77.7618677,45.1363155 77.3800778,45.5234254 76.9129961,45.5234254 L3.06957198,45.5234254 C2.60280156,45.5234254 2.22101167,45.1363155 2.22101167,44.6633915 L2.22101167,31.8651042 C2.22101167,31.3915493 2.60280156,31.0044394 3.06957198,31.0044394 L76.9129961,31.0044394 L76.9129961,31.0044394 Z" id="Fill-8" fill="#1A1919"></path>
                                                                        <polyline id="Fill-9" fill="#1A1919" points="6.14303502 41.5573634 6.14303502 34.9709746 7.86038911 34.9709746 7.86038911 40.005138 9.53182879 40.005138 9.53182879 34.9709746 11.248716 34.9709746 11.248716 40.005138 12.9203113 40.005138 12.9203113 34.9709746 14.637821 34.9709746 14.637821 41.5573634 6.14303502 41.5573634"></polyline>
                                                                        <polyline id="Fill-10" fill="#1A1919" points="15.8607004 41.5573634 15.8607004 34.9709746 17.5780545 34.9709746 17.5780545 37.464631 18.3953307 37.464631 19.8711284 34.9709746 21.7743191 34.9709746 19.7321401 38.1787493 21.867393 41.5573634 19.9642023 41.5573634 18.3856809 39.0170141 17.5780545 39.0170141 17.5780545 41.5573634 15.8607004 41.5573634"></polyline>
                                                                        <path d="M25.2449805,36.4291831 C24.1218677,36.4291831 23.5277821,37.2103437 23.5277821,38.2640901 C23.5277821,39.3175211 24.1218677,40.0986817 25.2449805,40.0986817 C26.368249,40.0986817 26.9623346,39.3175211 26.9623346,38.2640901 C26.9623346,37.2103437 26.368249,36.4291831 25.2449805,36.4291831 Z M25.2449805,34.8293183 C27.5662257,34.8293183 28.72607,36.4291831 28.72607,38.2640901 C28.72607,40.0986817 27.5662257,41.6985465 25.2449805,41.6985465 C22.9242023,41.6985465 21.7638911,40.0986817 21.7638911,38.2640901 C21.7638911,36.4291831 22.9242023,34.8293183 25.2449805,34.8293183 L25.2449805,34.8293183 Z" id="Fill-11" fill="#1A1919"></path>
                                                                        <path d="M35.2700389,34.9709746 L35.2700389,41.5573634 L33.5520623,41.5573634 L33.5520623,36.5233577 L32.1505058,36.5233577 C31.9461479,39.2238197 31.621323,41.6327662 29.7554864,41.6420732 C29.51393,41.6420732 29.2445136,41.6042141 28.9565759,41.5105127 L28.9565759,39.9955155 C29.0776654,40.0329014 29.2261479,40.051831 29.402179,40.0329014 C30.4513619,39.9202704 30.4329961,36.9655211 30.5814786,34.9709746 L35.2700389,34.9709746" id="Fill-12" fill="#1A1919"></path>
                                                                        <path d="M38.5357198,39.1107155 L40.2066926,39.1107155 L39.538677,37.2103437 C39.4828016,37.050231 39.4269261,36.8901183 39.3716732,36.5422873 C39.3154864,36.8901183 39.2596109,37.050231 39.204358,37.2103437 L38.5357198,39.1107155 Z M35.8896498,41.5573634 L38.4429572,34.9709746 L40.2997665,34.9709746 L42.8530739,41.5573634 L41.0893385,41.5573634 L40.7175097,40.5219155 L38.025214,40.5219155 L37.6540078,41.5573634 L35.8896498,41.5573634 L35.8896498,41.5573634 Z" id="Fill-13" fill="#1A1919"></path>
                                                                        <polyline id="Fill-14" fill="#1A1919" points="53.2258366 34.9709746 53.2258366 41.5573634 51.5083268 41.5573634 51.5083268 38.0568113 50.1159533 40.2207775 49.1319844 40.2207775 47.7392996 38.0471887 47.7392996 41.5573634 46.0214786 41.5573634 46.0214786 34.9709746 47.6462257 34.9709746 49.6238132 38.0568113 51.6014008 34.9709746 53.2258366 34.9709746"></polyline>
                                                                        <path d="M57.0113619,38.0004958 L58.0141634,38.0004958 L58.0141634,36.4758761 L57.0113619,36.4758761 C56.4264591,36.4758761 56.092607,36.7303211 56.092607,37.238107 C56.092607,37.7466817 56.4264591,38.0004958 57.0113619,38.0004958 Z M53.9203113,41.5573634 L55.6740856,39.1764958 C54.894786,38.8848225 54.3288716,38.2737127 54.3288716,37.238107 C54.3288716,35.5915493 55.5819455,34.9709746 56.9926848,34.9709746 L59.7321401,34.9709746 L59.7321401,41.5573634 L58.0141634,41.5573634 L58.0141634,39.4116958 L57.4107393,39.4116958 L55.8787549,41.5573634 L53.9203113,41.5573634 L53.9203113,41.5573634 Z" id="Fill-15" fill="#1A1919"></path>
                                                                        <path d="M64.5388327,41.5573634 L64.5388327,39.3175211 C64.0096498,39.478107 63.4994553,39.5623437 63.0351751,39.5623437 C61.6238132,39.5623437 60.6675486,38.828507 60.6675486,37.2849577 L60.6675486,34.9709746 L62.3849027,34.9709746 L62.3849027,37.1914141 C62.3849027,37.7277521 62.8585214,37.991031 63.4715953,37.991031 C63.8054475,37.991031 64.1774319,37.9162592 64.5388327,37.765769 L64.5388327,34.9709746 L66.256965,34.9709746 L66.256965,41.5573634 L64.5388327,41.5573634" id="Fill-16" fill="#1A1919"></path>
                                                                        <path d="M69.5232685,39.1107155 L71.1940856,39.1107155 L70.5256031,37.2103437 C70.4701946,37.050231 70.4138521,36.8901183 70.3584436,36.5422873 C70.3021012,36.8901183 70.2466926,37.050231 70.1911284,37.2103437 L69.5232685,39.1107155 Z M66.8770428,41.5573634 L69.4297276,34.9709746 L71.286537,34.9709746 L73.8398444,41.5573634 L72.0759533,41.5573634 L71.7045914,40.5219155 L69.0119844,40.5219155 L68.6414008,41.5573634 L66.8770428,41.5573634 L66.8770428,41.5573634 Z" id="Fill-17" fill="#1A1919"></path>
                                                                        <polyline id="Fill-18" fill="#1A1919" points="39.7543969 3.96353803 39.7094163 4.05077183 39.6155642 4.06733521 39.0719066 4.15030986 39.4681712 4.54735775 39.5336965 4.61376901 39.5171984 4.70936338 39.4272374 5.26873239 39.9056809 5.00734648 39.9915953 4.96238873 40.0771984 5.00734648 40.555642 5.26873239 40.4655253 4.70936338 40.4494942 4.61376901 40.5147082 4.54735775 40.9111284 4.15030986 40.3673152 4.06733521 40.273463 4.05077183 40.2284825 3.96353803 39.9915953 3.46206197 39.7543969 3.96353803"></polyline>
                                                                        <polyline id="Fill-19" fill="#1A1919" points="28.8933852 6.75328451 28.8929183 6.8520338 28.8168093 6.91008451 28.3718288 7.23661972 28.9033463 7.4055662 28.9915953 7.43396056 29.0203891 7.52655775 29.193463 8.06526197 29.5005447 7.61063662 29.5559533 7.53065915 29.6530739 7.53113239 30.1964202 7.54075493 29.863035 7.08549859 29.8057588 7.00851831 29.8333074 6.91891831 30.0057588 6.38131831 29.4852918 6.56036056 29.3945525 6.58938592 29.3151751 6.53307042 28.8776654 6.19754366 28.8933852 6.75328451"></polyline>
                                                                        <polyline id="Fill-20" fill="#1A1919" points="20.5050584 14.281262 20.5492607 14.368969 20.5080156 14.4562028 20.2597665 14.9534197 20.8088716 14.8562479 20.9000778 14.8407887 20.9676265 14.9092507 21.3651362 15.3078761 21.4323735 14.7612845 21.4457588 14.6641127 21.5316732 14.6197859 22.0188327 14.3754366 21.5168872 14.1258817 21.4308171 14.0837634 21.4150973 13.9913239 21.3252918 13.4335324 20.9433463 13.8348394 20.8761089 13.9031437 20.7796109 13.8894197 20.2392218 13.7950873 20.5050584 14.281262"></polyline>
                                                                        <polyline id="Fill-21" fill="#1A1919" points="16.4572763 24.8703099 16.5361868 24.9277296 16.5388327 25.0237972 16.5431907 25.5815887 16.9870817 25.2395944 17.061323 25.1834366 17.1523735 25.2129352 17.6859144 25.3823549 17.4985214 24.8655775 17.4663035 24.773138 17.5224903 24.6936338 17.8449805 24.2500507 17.2859144 24.2615662 17.1906615 24.2645634 17.134786 24.1899493 16.8026459 23.7357972 16.6446693 24.2699268 16.6157198 24.3615775 16.5238911 24.3943887 16.0007782 24.5614423 16.4572763 24.8703099"></polyline>
                                                                        <polyline id="Fill-22" fill="#1A1919" points="51.1055253 6.19754366 50.6680156 6.53307042 50.5884825 6.58938592 50.4978988 6.56036056 49.9768093 6.38131831 50.1492607 6.91891831 50.1774319 7.00851831 50.12 7.08549859 49.7867704 7.54075493 50.3299611 7.53113239 50.4264591 7.53065915 50.4828016 7.61063662 50.7897276 8.06526197 50.9623346 7.52655775 50.9914397 7.43396056 51.0792218 7.4055662 51.6113619 7.23661972 51.1657588 6.91008451 51.0896498 6.8520338 51.0891829 6.75328451 51.1055253 6.19754366"></polyline>
                                                                        <polyline id="Fill-23" fill="#1A1919" points="59.7436576 13.7950873 59.2032685 13.8894197 59.1067704 13.9031437 59.0393774 13.8348394 58.6575875 13.4335324 58.5677821 13.9913239 58.5519066 14.0837634 58.4661479 14.1258817 57.9640467 14.3754366 58.4512062 14.6197859 58.5371206 14.6641127 58.5506615 14.7612845 58.6177432 15.3078761 59.0152529 14.9092507 59.0826459 14.8407887 59.1738521 14.8562479 59.7231128 14.9534197 59.4748638 14.4562028 59.4336187 14.368969 59.477821 14.281262 59.7436576 13.7950873"></polyline>
                                                                        <polyline id="Fill-24" fill="#1A1919" points="63.9821012 24.5614423 63.4589883 24.3943887 63.3671595 24.3615775 63.3380545 24.2699268 63.1803891 23.7357972 62.8480934 24.1899493 62.7922179 24.2645634 62.696965 24.2615662 62.1377432 24.2500507 62.4603891 24.6936338 62.5165759 24.773138 62.484358 24.8655775 62.296965 25.3823549 62.8306615 25.2129352 62.9215564 25.1834366 62.9957977 25.2395944 63.4396887 25.5815887 63.4440467 25.0237972 63.4466926 24.9277296 63.5256031 24.8703099 63.9821012 24.5614423"></polyline>
                                                                        <path d="M33.7590661,50.1988732 C34.1280934,49.8556169 34.3410117,49.5669408 34.3410117,49.1086873 C34.3410117,48.4792789 33.8994553,48.0886986 33.1877043,48.0886986 C32.8578988,48.0886986 32.5262257,48.1656789 32.2789105,48.2991324 L32.2099611,48.3363606 L32.4456031,48.9552 L32.5344747,48.9037746 C32.6941634,48.8111775 32.913463,48.754231 33.1067704,48.754231 C33.3316732,48.754231 33.5993774,48.8141746 33.5993774,49.1004845 C33.5993774,49.3304789 33.5064591,49.4987944 33.2319066,49.7666479 C32.8871595,50.0998085 32.5856809,50.3515718 32.3101946,50.5378704 L32.2708171,50.5645296 L32.2708171,51.0623775 L34.3878599,51.0623775 L34.3878599,50.4050479 L33.5123735,50.4050479 C33.5929961,50.3444732 33.6754864,50.2771155 33.7590661,50.1988732" id="Fill-25" fill="#1A1919"></path>
                                                                        <path d="M37.6916732,50.444169 C37.2287938,50.444169 36.892607,50.0890817 36.892607,49.5995944 C36.892607,49.1099493 37.2287938,48.754231 37.6916732,48.754231 C38.1547082,48.754231 38.4902724,49.1099493 38.4902724,49.5995944 C38.4902724,50.0890817 38.1547082,50.444169 37.6916732,50.444169 Z M37.6916732,48.0886986 C36.8136965,48.0886986 36.1511284,48.7382986 36.1511284,49.5995944 C36.1511284,50.4602592 36.8136965,51.1098592 37.6916732,51.1098592 C38.5699611,51.1098592 39.2323735,50.4602592 39.2323735,49.5995944 C39.2323735,48.7382986 38.5699611,48.0886986 37.6916732,48.0886986 L37.6916732,48.0886986 Z" id="Fill-26" fill="#1A1919"></path>
                                                                        <path d="M42.4210117,50.444169 C41.9582879,50.444169 41.6224125,50.0890817 41.6224125,49.5995944 C41.6224125,49.1099493 41.9582879,48.754231 42.4210117,48.754231 C42.8837354,48.754231 43.2199222,49.1099493 43.2199222,49.5995944 C43.2199222,50.0890817 42.8837354,50.444169 42.4210117,50.444169 Z M42.4210117,48.0886986 C41.5424125,48.0886986 40.8804669,48.7382986 40.8804669,49.5995944 C40.8804669,50.4602592 41.5424125,51.1098592 42.4210117,51.1098592 C43.298677,51.1098592 43.9612451,50.4602592 43.9612451,49.5995944 C43.9612451,48.7382986 43.298677,48.0886986 42.4210117,48.0886986 L42.4210117,48.0886986 Z" id="Fill-27" fill="#1A1919"></path>
                                                                        <polyline id="Fill-28" fill="#1A1919" points="45.6678599 48.1360225 45.6678599 48.7933521 46.9361868 48.7933521 45.9906615 51.0623775 46.7833463 51.0623775 47.76607 48.620462 47.772607 48.1360225 45.6678599 48.1360225"></polyline>
                                                                        <path d="M74.8412451,32.6517859 C74.1543969,32.6517859 73.4592996,33.1103549 73.4592996,33.9869521 C73.4592996,34.863707 74.1543969,35.3222761 74.8412451,35.3222761 C75.5262257,35.3222761 76.2194553,34.863707 76.2194553,33.9869521 C76.2194553,33.1103549 75.5262257,32.6517859 74.8412451,32.6517859 Z M74.8412451,35.637769 C73.9998444,35.637769 73.1480156,35.0706704 73.1480156,33.9869521 C73.1480156,32.9033915 73.9998444,32.336293 74.8412451,32.336293 C75.6807782,32.336293 76.5307393,32.9033915 76.5307393,33.9869521 C76.5307393,35.0706704 75.6807782,35.637769 74.8412451,35.637769 L74.8412451,35.637769 Z" id="Fill-29" fill="#1A1919"></path>
                                                                        <path d="M74.9263813,33.925431 L74.5985992,33.925431 L74.5985992,33.5357972 L74.9263813,33.5357972 C75.0396887,33.5357972 75.193463,33.556462 75.193463,33.7326648 C75.193463,33.9050817 75.0396887,33.925431 74.9263813,33.925431 Z M75.2703502,34.2043268 C75.4566537,34.1470648 75.6063813,33.9869521 75.6063813,33.7162592 C75.6063813,33.3143211 75.2703502,33.1666704 74.9749416,33.1666704 L74.2101167,33.1666704 L74.2101167,34.7704789 L74.5985992,34.7704789 L74.5985992,34.2454986 L74.8333074,34.2454986 C74.8454475,34.2454986 74.8575875,34.2454986 74.8697276,34.2454986 L75.2582101,34.7704789 L75.6992996,34.7704789 L75.2703502,34.2043268 L75.2703502,34.2043268 Z" id="Fill-30" fill="#1A1919"></path>
                                                                    </g>
                                                                </g>
                                                            </g>
                                                        </g>
                                                    </g>
                                                </g>
                                            </g>
                                        </svg>
                                        </a>
                                      </div>
                                      */
                ?>
                <div class="col-md-1 mainHeaderLogo text-center">
                    <a href="{{ URL::to('/') }}">
                        <img src="{{ asset('assets/sb-ui/logo-white.svg')}}" style="width: 65%;
    margin: 0 20px;
    margin-top: 6px;">
                    </a>
                </div>

                <div class="col-md-7" style="padding-left: 30px;">

                    <div class="row">

                        <div class="col-auto text-center mainHeaderMenu" >
                            <h3><a href="/">ГЛАВНАЯ</a></h3>
                        </div>

                        <div class="col-auto text-center mainHeaderMenu" >
                            <h3><a href="/home#methods">МЕТОДИКА</a></h3>
                        </div>
                        <div class="col-auto text-center mainHeaderMenu" >
                            <h3><a href="/home#coaches">ТРЕНЕРЫ</a></h3>
                        </div>
                        <div class="col-auto text-center mainHeaderMenu">
                            <h3><a href="/clubs">КЛУБЫ</a></h3>
                        </div>
                        <div class="col-auto text-center mainHeaderMenu">
                            <h3><a href="/camp/cao">ЛАГЕРЯ</a></h3>
                        </div>

                        <div class="col-auto text-center mainHeaderMenu">
                            <h3><a href="/home#reviews">ОТЗЫВЫ</a></h3>
                        </div>


                    </div>

                </div>


                <div class="col-md-4 text-right" style="padding-right: 40px;">

                    <?php /*
                        <button id="myElement" type="button" name="button" style="height:48px;width:48px;text-shadow: 0px 0 3px #25a5e4;color:#25a5e4;"
                        class="mainHeaderButtonFix tippy-btn" data-tippy-interactive="true" data-tippy-theme="light rounded" data-tippy-arrow="true" data-tippy-size="large" data-tippy-arrowTransform="scale(2)" data-tippy-animation="fade" data-tippy-distance="15" data-tippy-interactiveBorder="20" data-local><i class="fab fa-telegram-plane"></i></button>
                        <div id="feature__html" style="padding: 20px; width: 300px; background: #FFF; color: #000;" data-template>
                            <div class="text-center" >
                                <h5>Теперь у нас появился виртуальный помощник!</h5>
                                <img src="{{ asset('assets/images/QRCode_telegram.png') }}" alt="" style="max-width: 100%;">
                                <h5>Подключайся к нашему чат-боту в Telegram, который поможет тебе узнать больше о <b>"Школе мяча"</b></h5>
                            </div>
                        </div>
 */
                    ?>


                    <button type="button" name="button" class="mainHeaderButtonFix" data-toggle="modal" data-animation="bounce" data-target=".bs-example-modal-lg">Записаться на тренировку</button>
                </div>



            </div>
            <!-- </div> -->
            <!-- </div> -->

        </div>
    </div>
</div>

<main class="window-main-content" style="overflow: hidden;">
    @yield ('content')
</main>

<!-- mobile footer !-->

<div class="footer mob-background d-sm-block d-md-none" style="overflow: hidden; ">
    <div class="container">

        <div class="col-md-12">
            <div class="FooterMoscow24Interview-mob">
                <div class="row">
                    <div class="col-md-5" style="padding-right: 0px;">
                        <img src="{{ asset('assets/moscow24white.png') }}" alt="">
                    </div>
                    <div class="col-md-7" style="margin-left: 0px;margin-top: 15px; ">

                        <h3>Руководитель, Андрей Вершинин, <br> <a href="https://www.m24.ru/audios/Moskva-FM/27072018/137837" target="_blank" style="cursor: pointer; color: #2a5dde;">дал интервью на радио «Москва24»</a></h3>


                    </div>
                </div>
            </div>
        </div>

        <div class="row">
            <div class="col-md-5 text-left">
                    <span>
                    <h4 style="
    text-align: left!important;
">Остались вопросы? Звоните:</h4>
                    <a href="tel:+74951206006" style="color: #FFF;text-align: left!important;"><h3 style="
    text-align: left!important;
    font-size: 20px;
">+7 (495) 120-60-06</h3>
</a>
                    <h5 style="
    text-align: left!important;
">Ежедневно с 10:00 до 20:00</h5>
                    </span>

                <h6 style="
    text-align: left!important; font-size: 18px!important;
"><a href="mailto:info@school-ball.com" style="font-size: 18px !important;">info@school-ball.com</a></h6>
                <h5 class="text-left" style="margin-top:4px;">Для отзывов и предложений</h5>
                <p style="
    text-align: left!important;
">Присоединяйтесь к нам в соцсетях,<br> чтобы быть в курсе событий школы</p>
                <div class="row">
                    <div class="col-md-1 col-2 socialLinks">
                        <!-- <img src="http://127.0.0.1:8000/assets/vk 2.png" alt=""> -->
                        <a href="https://vk.com/school_ball_industry"><img src="{{ asset('assets/footer_icons/vk-2.svg')}}" alt=""></a>
                    </div>
                    <div class="col-md-1 col-2 socialLinks">
                        <!-- <img src="http://127.0.0.1:8000/assets/facebook 2.png" alt=""> -->
                        <a href="https://t.me/school_ball_industry"><img src="{{ asset('assets/footer_icons/telegram.svg')}}" style="width: 32px; height: 32px;" alt=""></a>
                    </div>
                    <div class="col-md-1 col-2 socialLinks">
                        <!-- <img src="http://127.0.0.1:8000/assets/youtube.png" alt=""> -->
                        <a href="https://www.youtube.com/@schoolballtop"> <img src="{{ asset('assets/footer_icons/youtube.svg')}}" alt=""></a>
                    </div>
                </div>
                <p style="margin-top: 15px;text-align: left!important;"><img src="{{ asset('assets/footer_icons/group-19.svg')}}" alt=""> <span style="margin-left: 5px;"> или по счету </span></p>
            </div>
            <div class=" ">
                <div class="row">



                    <div class="col-md-12 text-left">
                        <div class="">
                            <h6 style="color: #6e6d7a;
    text-align: left !important;
    padding: 16px;">
                                <hr style="margin-top: 1rem;
    margin-bottom: 1rem;
    border: 0;
    border-top: 1px solid #6e6d7a;">

                                ООО «Школа мяча плюс», Адрес: 121 087, г. Москва, ул. Филёвская, 4Б, пом. 163,<br>
                                ИНН 7707407900, КПП 770701001, банк: АО «АЛЬФА-БАНК», г. Москва<br>
                                р/с 40702810702390003476, кор/сч 30101810200000000593, БИК 044525593  <br><br> ИП Зубанова Екатерина Юрьевна. Адрес 121609, Москва г.,ул. Осенняя, д. 2, кв. 14. ИНН 77313497733, ОГРНИП 321774600421541, Расчётный счёт: 40802810801500194909, 40802810920000102288, 40802810220000102289, Корреспондентский счёт:30101810745374525104, в ООО "Банк Точка", БИК: 044525104 <br><br>
                                Взаимодействуя с сайтом и используя формы обратной связи, вы даёте <a href="#">согласие</a>
                                на обработку  персональных данных и соглашаетесь с политикой конф-ных данных

                            </h6></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- mobile footer !-->

<!-- desc footer !-->

<div class="footer d-none d-md-block" style="overflow: hidden; ">
    <div class="container">
        <div class="row">
            <div class="col-md-5 text-left">
                <h4>Остались вопросы? Звоните:</h4>
                <a href="tel:+74951206006" style=" color: #FFF;"><h3>+7 (495) 120-60-06</h3></a>
                <h5>Ежедневно с 10:00 до 20:00</h5>
                <h6><a href="mailto:info@school-ball.com">info@school-ball.com</a></h6>
                <h5 class="d-none d-md-block">Для отзывов и предложений</h5>
                <p>Присоединяйтесь к нам в соцсетях,<br> чтобы быть в курсе событий школы</p>
                <div class="row">
                    <div class="col-md-1 col-2 socialLinks">
                        <!-- <img src="{{ asset('assets/vk 2.png') }}" alt=""> -->
                        <a href="https://vk.com/school_ball_industry"><img src="{{ asset('assets/footer_icons/vk-2.svg') }}" alt=""></a>
                    </div>

                    <div class="col-md-1 col-2 socialLinks">
                        <!-- <img src="http://127.0.0.1:8000/assets/facebook 2.png" alt=""> -->
                        <a href="https://t.me/school_ball_industry"><img src="{{ asset('assets/footer_icons/telegram.svg')}}" style="width: 32px; height: 32px;" alt=""></a>
                    </div>
                    <div class="col-md-1 col-2 socialLinks">
                        <!-- <img src="http://127.0.0.1:8000/assets/youtube.png" alt=""> -->
                        <a href="https://www.youtube.com/@schoolballtop"> <img src="{{ asset('assets/footer_icons/youtube.svg')}}" alt=""></a>
                    </div>
                </div>
                <p style="margin-top: 15px;"><img src="{{ asset('assets/footer_icons/group-19.svg') }}" alt=""> <span style="margin-left: 5px;"> или по счету </span></p>
            </div>
            <div class="col-md-6 offset-1 text-left d-none d-md-block">
                <div class="row">
                    <div class="col-md-8" style="margin-left: -17px;
">
                        <div class="row">
                            <div class="col-md-6">


                                <div class="col-md-12 footerLinks">
                                    <h5><a style="font-size: 18px;  color: #fff; " href="#methods">Методика</a></h5>
                                </div>
                                <div class="col-md-12 footerLinks">
                                    <h5 ><a style="font-size: 18px;  color: #fff; " href="#coaches"> Тренеры </a></h5>
                                </div>

                            </div>

                            <div class="col-md-6">
                                <div class="col-md-12 footerLinks">
                                    <h5><a style="font-size: 18px;  color: #fff; " href="#clubsloc">Клубы</a></h5>
                                </div>

                                <div class="col-md-12 footerLinks">
                                    <h5><a style="font-size: 18px;  color: #fff; " href="#reviews">Отзывы</a></h5>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="w-100"></div>
                    <div class="col-md-12">
                        <div class="FooterMoscow24Interview">
                            <div class="row">
                                <div class="col-md-5" style="padding-right: 0px;">
                                    <img src="{{ asset('assets/moscow24white.png') }}" alt="">
                                </div>
                                <div class="col-md-7" style="margin-left: 0px; padding-left: 30px;">

                                    <h3>Руководитель, Андрей Вершинин, <br> <a href="https://www.m24.ru/audios/Moskva-FM/27072018/137837" target="_blank" style="cursor: pointer; color: #2a5dde;">дал интервью на радио «Москва24»</a></h3>


                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-12 text-left">
                        <div class="footerPersonalData">
                            <h6>
                                ООО «Школа мяча плюс», Адрес: 121 087, г. Москва, ул. Филёвская, 4Б, пом. 163,<br>
                                ИНН 7707407900, КПП 770701001, банк: АО «АЛЬФА-БАНК», г. Москва<br>
                                р/с 40702810702390003476, кор/сч 30101810200000000593, БИК 044525593  <br><br> ИП Зубанова Екатерина Юрьевна. Адрес 121609, Москва г.,ул. Осенняя, д. 2, кв. 14. ИНН 77313497733, ОГРНИП 321774600421541, Расчётный счёт: 40802810801500194909, 40802810920000102288, 40802810220000102289, Корреспондентский счёт:30101810745374525104, в ООО "Банк Точка", БИК: 044525104 <br><br>
                                Взаимодействуя с сайтом и используя формы обратной связи, вы даёте <a href="#">согласие</a>
                                на обработку  персональных данных и соглашаетесь с политикой конф-ных данных

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- desc footer !-->

<!--  Modal content for the above example -->

<!--  Modal content for the above example -->

<?php /*
    <div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg">
              <div class="modal-content">
                  <div class="modal-header text-center">
                      <h5 class="modal-title mt-0" id="myLargeModalLabel"style="font-family: 'IBMPlexSans-Bold';">Оставить заявку</h5>
                      <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="color: #343a40;width: unset;height: unset;background: unset;border: unset;font-size: 1.5rem;line-height: 1;">×</button>
                  </div>
                  <div id="callback-1-front">
                    <div class="modal-body text-center">
                      <div class="row">
                        <div class="col-md-12">
                          <svg width="200px" height="200px" viewBox="0 0 80 56" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                              <defs>
                                  <rect id="path-1" x="0" y="0" width="1216" height="72" rx="8"></rect>
                                  <filter x="-3.0%" y="-51.4%" width="106.1%" height="202.8%" filterUnits="objectBoundingBox" id="filter-2">
                                      <feMorphology radius="1" operator="dilate" in="SourceAlpha" result="shadowSpreadOuter1"></feMorphology>
                                      <feOffset dx="0" dy="0" in="shadowSpreadOuter1" result="shadowOffsetOuter1"></feOffset>
                                      <feGaussianBlur stdDeviation="12" in="shadowOffsetOuter1" result="shadowBlurOuter1"></feGaussianBlur>
                                      <feComposite in="shadowBlurOuter1" in2="SourceAlpha" operator="out" result="shadowBlurOuter1"></feComposite>
                                      <feColorMatrix values="0 0 0 0 0.323977461   0 0 0 0 0.381127872   0 0 0 0 0.466853487  0 0 0 0.15 0" type="matrix" in="shadowBlurOuter1"></feColorMatrix>
                                  </filter>
                              </defs>
                              <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                  <g id="Menu" transform="translate(-32.000000, -56.000000)">
                                      <g id="Group-76">
                                          <g id="Group-45" transform="translate(0.000000, 48.000000)">
                                              <g id="Rectangle" opacity="0.600000024">
                                                  <use fill="black" fill-opacity="1" filter="url(#filter-2)" xlink:href="#path-1"></use>
                                                  <rect stroke="#FFFFFF" stroke-width="1" fill="#FFFFFF" fill-rule="evenodd" x="-0.5" y="-0.5" width="1217" height="73" rx="8"></rect>
                                              </g>
                                              <g id="SchoolBall_Logo_Rus-3" transform="translate(32.000000, 8.000000)">
                                                  <g id="SchoolBall_Logo_Rus-2">
                                                      <g id="SchoolBall_Logo_Rus">
                                                          <path d="M76.9129961,28.8800676 L3.06957198,28.8800676 C1.44607004,28.8800676 0.124980545,30.2190197 0.124980545,31.8651042 L0.124980545,44.6633915 C0.124980545,46.3086873 1.44607004,47.6477972 3.06957198,47.6477972 L76.9129961,47.6477972 C78.5368093,47.6477972 79.8578988,46.3086873 79.8578988,44.6633915 L79.8578988,31.8651042 C79.8578988,30.2190197 78.5368093,28.8800676 76.9129961,28.8800676" id="Fill-3" fill="#FFFFFE"></path>
                                                          <path d="M39.991284,0.0900732394 C24.7950195,0.0900732394 12.4322179,12.6203493 12.4322179,28.0225577 C12.4322179,43.4238197 24.7950195,55.9540958 39.991284,55.9540958 C55.1875486,55.9540958 67.5506615,43.4238197 67.5506615,28.0225577 C67.5506615,12.6203493 55.1875486,0.0900732394 39.991284,0.0900732394" id="Fill-4" fill="#FFFFFE"></path>
                                                          <path d="M25.4898054,13.8178028 C25.5634241,13.7376676 25.6406226,13.6594254 25.7184436,13.5807099 C28.6759533,10.5832113 32.4,8.62242254 36.4992996,7.89931268 L36.9148638,7.82580282 L36.6375097,8.14823662 C36.2084047,8.6484507 35.8720623,9.07468169 35.6381323,9.41494085 C35.5382101,9.56196056 35.4014008,9.76292958 35.3047471,9.97983099 L35.2729961,10.0516056 L35.1971984,10.0662761 C35.15393,10.0752676 30.8289494,10.9584901 26.9277821,14.281262 L26.8731518,14.3274817 L25.3067704,14.0173521 L25.4898054,13.8178028 Z M40.8779767,13.9766535 L40.9582879,14.0340732 L39.52607,20.9575662 L39.4552529,20.9891155 C35.5094163,22.7276394 33.0269261,24.3243493 32.1427237,24.9300958 L32.065214,24.9835718 L26.5909728,21.7887324 L26.5985992,21.6928225 C26.6641245,20.8805859 27.0138521,16.7534648 27.5021012,15.245093 L27.5150195,15.2053408 L27.5466148,15.1780507 C31.174786,11.9851042 35.6610117,11.1113465 35.7056809,11.1025127 L35.8387549,11.0952563 L35.8983658,11.1201803 C36.4037354,11.3626366 38.9827237,12.6198761 40.8779767,13.9766535 Z M42.0936965,14.1526986 C43.498677,13.629138 47.1842802,13.2753127 49.1402335,13.1486423 L49.1878599,13.1454873 L49.2284825,13.1710423 C52.8908949,15.4660958 54.8018677,18.817893 54.820856,18.8511775 L54.8470039,18.8980282 L54.8387549,18.9500845 C54.6731518,19.9968901 54.0747082,23.5651155 53.3077043,25.5514592 L53.2792218,25.6254423 L46.5797665,27.2590648 L40.5696498,21.168 L42.0158755,14.1809352 L42.0936965,14.1526986 Z M49.1047471,9.79021972 C51.0017121,10.7616225 52.7322957,12.0324282 54.2485603,13.5693521 C55.7802335,15.1211042 57.0459144,16.8962254 58.010428,18.8451831 L58.1794553,19.1867042 L55.7273152,18.2798197 L55.7000778,18.2336 C55.6790661,18.1979493 53.5637354,14.6451831 49.8311284,12.2783549 L49.789572,12.2523268 L48.7523735,9.60991549 L49.1047471,9.79021972 Z M20.5070817,33.2906592 C19.8263035,30.6968338 19.6544747,27.9804394 20.0152529,25.2739831 L20.0761089,24.821093 L21.0810895,26.6597859 L21.0761089,26.7078986 C20.825214,29.0188845 21.0954086,31.4554366 21.4205447,33.2906592 L22.5055253,33.2906592 C22.1715175,31.4715268 21.8835798,29.0272451 22.140856,26.7556958 L22.1598444,26.6970141 C22.8719066,25.455707 24.5634241,23.0145803 25.8692607,22.6612282 L25.9290272,22.6452958 L31.6233463,25.9668056 L33.1310506,33.2906592 L44.0555642,33.2906592 L46.7670039,28.3218028 L53.4561868,26.6924394 L53.5153307,26.7717859 C54.56,28.1758873 56.3905058,30.8705127 56.6205447,32.064969 L56.627393,32.099831 L56.6245914,32.1105577 C56.6227237,32.2326535 56.6040467,32.6416901 56.4490272,33.2906592 L57.5431907,33.2906592 C57.6038911,33.0007211 57.6396887,32.7588958 57.6602335,32.5711775 L57.6706615,32.4744789 L57.7612451,32.4435606 C58.2239689,32.2872338 58.9228016,31.9446085 59.6270817,31.2030423 L59.9596887,30.8533183 L59.8824903,31.3322366 C59.7758755,31.9955606 59.6347082,32.6481577 59.4672374,33.2906592 L60.5662257,33.2906592 C60.9433463,31.7643042 61.1626459,30.1882592 61.2032685,28.5819268 C61.2062257,28.5066817 61.2079377,28.4320676 61.2079377,28.3561915 C61.2521401,25.4850479 60.7453696,22.6855211 59.700856,20.0330141 L59.629572,19.8505014 C58.5657588,17.2115606 57.0084047,14.841893 55.0009339,12.8071211 C52.9953307,10.7747155 50.6578988,9.19677746 48.0521401,8.11747606 L47.9038132,8.05579718 C45.3845914,7.03454648 42.7256031,6.51587606 40.0015564,6.51587606 C39.4555642,6.51587606 38.9031907,6.53780282 38.3592218,6.58055211 L38.3511284,6.5811831 C38.3494163,6.5811831 38.3478599,6.5811831 38.3461479,6.5811831 L38.3142412,6.58402254 L38.3101946,6.58228732 C38.2518288,6.58528451 38.1943969,6.59049014 38.1397665,6.59711549 C33.1702724,7.03454648 28.4975875,9.23936901 24.9772763,12.8071211 C24.6073152,13.181769 24.2476265,13.5740845 23.9086381,13.9717634 C23.8775097,14.0086761 23.8627237,14.0257127 23.8488716,14.043538 C21.6714397,16.6212732 20.1157977,19.7254085 19.3450584,23.0257803 C19.3270039,23.095031 19.311751,23.1608113 19.2964981,23.2284845 L19.2930739,23.2504113 C18.5508171,26.5791775 18.6163424,30.0294085 19.4225681,33.2906592 L20.5070817,33.2906592 L20.5070817,33.2906592 Z" id="Fill-5" fill="#1A1919"></path>
                                                          <path d="M39.991284,1.15233803 C25.3729183,1.15233803 13.4801556,13.206062 13.4801556,28.0225577 C13.4801556,42.8382648 25.3729183,54.8921465 39.991284,54.8921465 C54.6096498,54.8921465 66.5025681,42.8382648 66.5025681,28.0225577 C66.5025681,13.206062 54.6096498,1.15233803 39.991284,1.15233803 Z M39.991284,2.21444507 C54.0541634,2.21444507 65.4546304,13.7684282 65.4546304,28.0225577 C65.4546304,42.2751099 54.0541634,53.8297239 39.991284,53.8297239 C25.9284047,53.8297239 14.528249,42.2751099 14.528249,28.0225577 C14.528249,13.7684282 25.9284047,2.21444507 39.991284,2.21444507 L39.991284,2.21444507 Z" id="Fill-6" fill="#1A1919"></path>
                                                          <path d="M3.06957198,46.0544 C2.31268482,46.0544 1.69696498,45.4301972 1.69696498,44.6633915 L1.69696498,31.8651042 C1.69696498,31.0975099 2.31268482,30.4734648 3.06957198,30.4734648 L76.9129961,30.4734648 C77.6700389,30.4734648 78.2859144,31.0975099 78.2859144,31.8651042 L78.2859144,44.6633915 C78.2859144,45.4301972 77.6700389,46.0544 76.9129961,46.0544 L3.06957198,46.0544" id="Fill-7" fill="#F0A936"></path>
                                                          <path d="M76.9129961,29.9423324 L3.06957198,29.9423324 C2.02381323,29.9423324 1.17291829,30.8047324 1.17291829,31.8651042 L1.17291829,44.6633915 C1.17291829,45.7231324 2.02381323,46.5855324 3.06957198,46.5855324 L76.9129961,46.5855324 C77.9590661,46.5855324 78.8099611,45.7231324 78.8099611,44.6633915 L78.8099611,31.8651042 C78.8099611,30.8047324 77.9590661,29.9423324 76.9129961,29.9423324 Z M76.9129961,31.0044394 C77.3800778,31.0044394 77.7618677,31.3915493 77.7618677,31.8651042 L77.7618677,44.6633915 C77.7618677,45.1363155 77.3800778,45.5234254 76.9129961,45.5234254 L3.06957198,45.5234254 C2.60280156,45.5234254 2.22101167,45.1363155 2.22101167,44.6633915 L2.22101167,31.8651042 C2.22101167,31.3915493 2.60280156,31.0044394 3.06957198,31.0044394 L76.9129961,31.0044394 L76.9129961,31.0044394 Z" id="Fill-8" fill="#1A1919"></path>
                                                          <polyline id="Fill-9" fill="#1A1919" points="6.14303502 41.5573634 6.14303502 34.9709746 7.86038911 34.9709746 7.86038911 40.005138 9.53182879 40.005138 9.53182879 34.9709746 11.248716 34.9709746 11.248716 40.005138 12.9203113 40.005138 12.9203113 34.9709746 14.637821 34.9709746 14.637821 41.5573634 6.14303502 41.5573634"></polyline>
                                                          <polyline id="Fill-10" fill="#1A1919" points="15.8607004 41.5573634 15.8607004 34.9709746 17.5780545 34.9709746 17.5780545 37.464631 18.3953307 37.464631 19.8711284 34.9709746 21.7743191 34.9709746 19.7321401 38.1787493 21.867393 41.5573634 19.9642023 41.5573634 18.3856809 39.0170141 17.5780545 39.0170141 17.5780545 41.5573634 15.8607004 41.5573634"></polyline>
                                                          <path d="M25.2449805,36.4291831 C24.1218677,36.4291831 23.5277821,37.2103437 23.5277821,38.2640901 C23.5277821,39.3175211 24.1218677,40.0986817 25.2449805,40.0986817 C26.368249,40.0986817 26.9623346,39.3175211 26.9623346,38.2640901 C26.9623346,37.2103437 26.368249,36.4291831 25.2449805,36.4291831 Z M25.2449805,34.8293183 C27.5662257,34.8293183 28.72607,36.4291831 28.72607,38.2640901 C28.72607,40.0986817 27.5662257,41.6985465 25.2449805,41.6985465 C22.9242023,41.6985465 21.7638911,40.0986817 21.7638911,38.2640901 C21.7638911,36.4291831 22.9242023,34.8293183 25.2449805,34.8293183 L25.2449805,34.8293183 Z" id="Fill-11" fill="#1A1919"></path>
                                                          <path d="M35.2700389,34.9709746 L35.2700389,41.5573634 L33.5520623,41.5573634 L33.5520623,36.5233577 L32.1505058,36.5233577 C31.9461479,39.2238197 31.621323,41.6327662 29.7554864,41.6420732 C29.51393,41.6420732 29.2445136,41.6042141 28.9565759,41.5105127 L28.9565759,39.9955155 C29.0776654,40.0329014 29.2261479,40.051831 29.402179,40.0329014 C30.4513619,39.9202704 30.4329961,36.9655211 30.5814786,34.9709746 L35.2700389,34.9709746" id="Fill-12" fill="#1A1919"></path>
                                                          <path d="M38.5357198,39.1107155 L40.2066926,39.1107155 L39.538677,37.2103437 C39.4828016,37.050231 39.4269261,36.8901183 39.3716732,36.5422873 C39.3154864,36.8901183 39.2596109,37.050231 39.204358,37.2103437 L38.5357198,39.1107155 Z M35.8896498,41.5573634 L38.4429572,34.9709746 L40.2997665,34.9709746 L42.8530739,41.5573634 L41.0893385,41.5573634 L40.7175097,40.5219155 L38.025214,40.5219155 L37.6540078,41.5573634 L35.8896498,41.5573634 L35.8896498,41.5573634 Z" id="Fill-13" fill="#1A1919"></path>
                                                          <polyline id="Fill-14" fill="#1A1919" points="53.2258366 34.9709746 53.2258366 41.5573634 51.5083268 41.5573634 51.5083268 38.0568113 50.1159533 40.2207775 49.1319844 40.2207775 47.7392996 38.0471887 47.7392996 41.5573634 46.0214786 41.5573634 46.0214786 34.9709746 47.6462257 34.9709746 49.6238132 38.0568113 51.6014008 34.9709746 53.2258366 34.9709746"></polyline>
                                                          <path d="M57.0113619,38.0004958 L58.0141634,38.0004958 L58.0141634,36.4758761 L57.0113619,36.4758761 C56.4264591,36.4758761 56.092607,36.7303211 56.092607,37.238107 C56.092607,37.7466817 56.4264591,38.0004958 57.0113619,38.0004958 Z M53.9203113,41.5573634 L55.6740856,39.1764958 C54.894786,38.8848225 54.3288716,38.2737127 54.3288716,37.238107 C54.3288716,35.5915493 55.5819455,34.9709746 56.9926848,34.9709746 L59.7321401,34.9709746 L59.7321401,41.5573634 L58.0141634,41.5573634 L58.0141634,39.4116958 L57.4107393,39.4116958 L55.8787549,41.5573634 L53.9203113,41.5573634 L53.9203113,41.5573634 Z" id="Fill-15" fill="#1A1919"></path>
                                                          <path d="M64.5388327,41.5573634 L64.5388327,39.3175211 C64.0096498,39.478107 63.4994553,39.5623437 63.0351751,39.5623437 C61.6238132,39.5623437 60.6675486,38.828507 60.6675486,37.2849577 L60.6675486,34.9709746 L62.3849027,34.9709746 L62.3849027,37.1914141 C62.3849027,37.7277521 62.8585214,37.991031 63.4715953,37.991031 C63.8054475,37.991031 64.1774319,37.9162592 64.5388327,37.765769 L64.5388327,34.9709746 L66.256965,34.9709746 L66.256965,41.5573634 L64.5388327,41.5573634" id="Fill-16" fill="#1A1919"></path>
                                                          <path d="M69.5232685,39.1107155 L71.1940856,39.1107155 L70.5256031,37.2103437 C70.4701946,37.050231 70.4138521,36.8901183 70.3584436,36.5422873 C70.3021012,36.8901183 70.2466926,37.050231 70.1911284,37.2103437 L69.5232685,39.1107155 Z M66.8770428,41.5573634 L69.4297276,34.9709746 L71.286537,34.9709746 L73.8398444,41.5573634 L72.0759533,41.5573634 L71.7045914,40.5219155 L69.0119844,40.5219155 L68.6414008,41.5573634 L66.8770428,41.5573634 L66.8770428,41.5573634 Z" id="Fill-17" fill="#1A1919"></path>
                                                          <polyline id="Fill-18" fill="#1A1919" points="39.7543969 3.96353803 39.7094163 4.05077183 39.6155642 4.06733521 39.0719066 4.15030986 39.4681712 4.54735775 39.5336965 4.61376901 39.5171984 4.70936338 39.4272374 5.26873239 39.9056809 5.00734648 39.9915953 4.96238873 40.0771984 5.00734648 40.555642 5.26873239 40.4655253 4.70936338 40.4494942 4.61376901 40.5147082 4.54735775 40.9111284 4.15030986 40.3673152 4.06733521 40.273463 4.05077183 40.2284825 3.96353803 39.9915953 3.46206197 39.7543969 3.96353803"></polyline>
                                                          <polyline id="Fill-19" fill="#1A1919" points="28.8933852 6.75328451 28.8929183 6.8520338 28.8168093 6.91008451 28.3718288 7.23661972 28.9033463 7.4055662 28.9915953 7.43396056 29.0203891 7.52655775 29.193463 8.06526197 29.5005447 7.61063662 29.5559533 7.53065915 29.6530739 7.53113239 30.1964202 7.54075493 29.863035 7.08549859 29.8057588 7.00851831 29.8333074 6.91891831 30.0057588 6.38131831 29.4852918 6.56036056 29.3945525 6.58938592 29.3151751 6.53307042 28.8776654 6.19754366 28.8933852 6.75328451"></polyline>
                                                          <polyline id="Fill-20" fill="#1A1919" points="20.5050584 14.281262 20.5492607 14.368969 20.5080156 14.4562028 20.2597665 14.9534197 20.8088716 14.8562479 20.9000778 14.8407887 20.9676265 14.9092507 21.3651362 15.3078761 21.4323735 14.7612845 21.4457588 14.6641127 21.5316732 14.6197859 22.0188327 14.3754366 21.5168872 14.1258817 21.4308171 14.0837634 21.4150973 13.9913239 21.3252918 13.4335324 20.9433463 13.8348394 20.8761089 13.9031437 20.7796109 13.8894197 20.2392218 13.7950873 20.5050584 14.281262"></polyline>
                                                          <polyline id="Fill-21" fill="#1A1919" points="16.4572763 24.8703099 16.5361868 24.9277296 16.5388327 25.0237972 16.5431907 25.5815887 16.9870817 25.2395944 17.061323 25.1834366 17.1523735 25.2129352 17.6859144 25.3823549 17.4985214 24.8655775 17.4663035 24.773138 17.5224903 24.6936338 17.8449805 24.2500507 17.2859144 24.2615662 17.1906615 24.2645634 17.134786 24.1899493 16.8026459 23.7357972 16.6446693 24.2699268 16.6157198 24.3615775 16.5238911 24.3943887 16.0007782 24.5614423 16.4572763 24.8703099"></polyline>
                                                          <polyline id="Fill-22" fill="#1A1919" points="51.1055253 6.19754366 50.6680156 6.53307042 50.5884825 6.58938592 50.4978988 6.56036056 49.9768093 6.38131831 50.1492607 6.91891831 50.1774319 7.00851831 50.12 7.08549859 49.7867704 7.54075493 50.3299611 7.53113239 50.4264591 7.53065915 50.4828016 7.61063662 50.7897276 8.06526197 50.9623346 7.52655775 50.9914397 7.43396056 51.0792218 7.4055662 51.6113619 7.23661972 51.1657588 6.91008451 51.0896498 6.8520338 51.0891829 6.75328451 51.1055253 6.19754366"></polyline>
                                                          <polyline id="Fill-23" fill="#1A1919" points="59.7436576 13.7950873 59.2032685 13.8894197 59.1067704 13.9031437 59.0393774 13.8348394 58.6575875 13.4335324 58.5677821 13.9913239 58.5519066 14.0837634 58.4661479 14.1258817 57.9640467 14.3754366 58.4512062 14.6197859 58.5371206 14.6641127 58.5506615 14.7612845 58.6177432 15.3078761 59.0152529 14.9092507 59.0826459 14.8407887 59.1738521 14.8562479 59.7231128 14.9534197 59.4748638 14.4562028 59.4336187 14.368969 59.477821 14.281262 59.7436576 13.7950873"></polyline>
                                                          <polyline id="Fill-24" fill="#1A1919" points="63.9821012 24.5614423 63.4589883 24.3943887 63.3671595 24.3615775 63.3380545 24.2699268 63.1803891 23.7357972 62.8480934 24.1899493 62.7922179 24.2645634 62.696965 24.2615662 62.1377432 24.2500507 62.4603891 24.6936338 62.5165759 24.773138 62.484358 24.8655775 62.296965 25.3823549 62.8306615 25.2129352 62.9215564 25.1834366 62.9957977 25.2395944 63.4396887 25.5815887 63.4440467 25.0237972 63.4466926 24.9277296 63.5256031 24.8703099 63.9821012 24.5614423"></polyline>
                                                          <path d="M33.7590661,50.1988732 C34.1280934,49.8556169 34.3410117,49.5669408 34.3410117,49.1086873 C34.3410117,48.4792789 33.8994553,48.0886986 33.1877043,48.0886986 C32.8578988,48.0886986 32.5262257,48.1656789 32.2789105,48.2991324 L32.2099611,48.3363606 L32.4456031,48.9552 L32.5344747,48.9037746 C32.6941634,48.8111775 32.913463,48.754231 33.1067704,48.754231 C33.3316732,48.754231 33.5993774,48.8141746 33.5993774,49.1004845 C33.5993774,49.3304789 33.5064591,49.4987944 33.2319066,49.7666479 C32.8871595,50.0998085 32.5856809,50.3515718 32.3101946,50.5378704 L32.2708171,50.5645296 L32.2708171,51.0623775 L34.3878599,51.0623775 L34.3878599,50.4050479 L33.5123735,50.4050479 C33.5929961,50.3444732 33.6754864,50.2771155 33.7590661,50.1988732" id="Fill-25" fill="#1A1919"></path>
                                                          <path d="M37.6916732,50.444169 C37.2287938,50.444169 36.892607,50.0890817 36.892607,49.5995944 C36.892607,49.1099493 37.2287938,48.754231 37.6916732,48.754231 C38.1547082,48.754231 38.4902724,49.1099493 38.4902724,49.5995944 C38.4902724,50.0890817 38.1547082,50.444169 37.6916732,50.444169 Z M37.6916732,48.0886986 C36.8136965,48.0886986 36.1511284,48.7382986 36.1511284,49.5995944 C36.1511284,50.4602592 36.8136965,51.1098592 37.6916732,51.1098592 C38.5699611,51.1098592 39.2323735,50.4602592 39.2323735,49.5995944 C39.2323735,48.7382986 38.5699611,48.0886986 37.6916732,48.0886986 L37.6916732,48.0886986 Z" id="Fill-26" fill="#1A1919"></path>
                                                          <path d="M42.4210117,50.444169 C41.9582879,50.444169 41.6224125,50.0890817 41.6224125,49.5995944 C41.6224125,49.1099493 41.9582879,48.754231 42.4210117,48.754231 C42.8837354,48.754231 43.2199222,49.1099493 43.2199222,49.5995944 C43.2199222,50.0890817 42.8837354,50.444169 42.4210117,50.444169 Z M42.4210117,48.0886986 C41.5424125,48.0886986 40.8804669,48.7382986 40.8804669,49.5995944 C40.8804669,50.4602592 41.5424125,51.1098592 42.4210117,51.1098592 C43.298677,51.1098592 43.9612451,50.4602592 43.9612451,49.5995944 C43.9612451,48.7382986 43.298677,48.0886986 42.4210117,48.0886986 L42.4210117,48.0886986 Z" id="Fill-27" fill="#1A1919"></path>
                                                          <polyline id="Fill-28" fill="#1A1919" points="45.6678599 48.1360225 45.6678599 48.7933521 46.9361868 48.7933521 45.9906615 51.0623775 46.7833463 51.0623775 47.76607 48.620462 47.772607 48.1360225 45.6678599 48.1360225"></polyline>
                                                          <path d="M74.8412451,32.6517859 C74.1543969,32.6517859 73.4592996,33.1103549 73.4592996,33.9869521 C73.4592996,34.863707 74.1543969,35.3222761 74.8412451,35.3222761 C75.5262257,35.3222761 76.2194553,34.863707 76.2194553,33.9869521 C76.2194553,33.1103549 75.5262257,32.6517859 74.8412451,32.6517859 Z M74.8412451,35.637769 C73.9998444,35.637769 73.1480156,35.0706704 73.1480156,33.9869521 C73.1480156,32.9033915 73.9998444,32.336293 74.8412451,32.336293 C75.6807782,32.336293 76.5307393,32.9033915 76.5307393,33.9869521 C76.5307393,35.0706704 75.6807782,35.637769 74.8412451,35.637769 L74.8412451,35.637769 Z" id="Fill-29" fill="#1A1919"></path>
                                                          <path d="M74.9263813,33.925431 L74.5985992,33.925431 L74.5985992,33.5357972 L74.9263813,33.5357972 C75.0396887,33.5357972 75.193463,33.556462 75.193463,33.7326648 C75.193463,33.9050817 75.0396887,33.925431 74.9263813,33.925431 Z M75.2703502,34.2043268 C75.4566537,34.1470648 75.6063813,33.9869521 75.6063813,33.7162592 C75.6063813,33.3143211 75.2703502,33.1666704 74.9749416,33.1666704 L74.2101167,33.1666704 L74.2101167,34.7704789 L74.5985992,34.7704789 L74.5985992,34.2454986 L74.8333074,34.2454986 C74.8454475,34.2454986 74.8575875,34.2454986 74.8697276,34.2454986 L75.2582101,34.7704789 L75.6992996,34.7704789 L75.2703502,34.2043268 L75.2703502,34.2043268 Z" id="Fill-30" fill="#1A1919"></path>
                                                      </g>
                                                  </g>
                                              </g>
                                          </g>
                                      </g>
                                  </g>
                              </g>
                        </div>
                      </div>
                      <form id="sendform" method="post">
                        @csrf
                        <div class="row">
                          <label for="name" class="col-md-4 col-form-label-new text-md-right">Как к Вам обращаться?</label>
                          <div class="col-md-6">
                            <input type="text" name="name" value="" class="form-control-new" placeholder="Петров Пётр">
                          </div>
                        </div>
                        <div class="row">
                          <label for="phone" class="col-md-4 col-form-label-new text-md-right">Ваш контактный телефон:</label>
                          <div class="col-md-6">
                            <input type="text" name="phone" value="" class="phone form-control-new">
                          </div>
                        </div>
                        <hr>
                        <div class="row">
                          <label for="name_kid" class="col-md-4 col-form-label-new text-md-right">ФИО ребенка:</label>
                          <div class="col-md-6">
                            <input type="text" name="name_kid" value="" class="form-control-new" placeholder="Иванов Иван">
                          </div>
                        </div>
                        <div class="row">
                          <label for="date-birth-kid" class="col-md-4 col-form-label-new text-md-right">Дата рождения:</label>
                          <div class="col-md-6">
                            <input id="date-birth" name="date-birth-kid" type="text" class="datepicker-here form-control-new">
                          </div>
                        </div>
                        <div class="row">
                          <label for="club" class="col-md-4 col-form-label-new text-md-right">Клуб:</label>
                          <div class="col-md-6">
                            <select class="form-control-new" name="club" style="margin-bottom: .5rem;">
                              <option value="0">Не выбрано</option>
                              @foreach ($clublist as $c)
                                <option value="{{ $c->id }}">{{ $c->name }}</option>
                              @endforeach
                            </select>
                          </div>
                        </div>
                      </form>
                    </div>
                    <div class="modal-footer text-center" >
                      <!-- <form action="?" method="POST" style="margin: auto;">
                        <div class="g-recaptcha" data-sitekey="6LcUwPYZAAAAAPB4chuH644sQULIA7lV6LjJL_w1"></div>
                        <br/> -->
                        <button type="submit" class="btn btn-primary waves-effect waves-light" style="margin: 0 auto; color: #54420a; background-color: #fede11; border: solid 1px #fede11; box-shadow: none;" onclick="sendForm()">Отправить заявку</button>
                      <!-- </form> -->
                    </div>
                  </div>
                  <div id="callback-1-done" class="d-none">
                    <div class="modal-body text-center">
                      <div class="row">
                        <div class="col-md-12">
                          <svg width="200px" height="200px" viewBox="0 0 80 56" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                              <defs>
                                  <rect id="path-1" x="0" y="0" width="1216" height="72" rx="8"></rect>
                                  <filter x="-3.0%" y="-51.4%" width="106.1%" height="202.8%" filterUnits="objectBoundingBox" id="filter-2">
                                      <feMorphology radius="1" operator="dilate" in="SourceAlpha" result="shadowSpreadOuter1"></feMorphology>
                                      <feOffset dx="0" dy="0" in="shadowSpreadOuter1" result="shadowOffsetOuter1"></feOffset>
                                      <feGaussianBlur stdDeviation="12" in="shadowOffsetOuter1" result="shadowBlurOuter1"></feGaussianBlur>
                                      <feComposite in="shadowBlurOuter1" in2="SourceAlpha" operator="out" result="shadowBlurOuter1"></feComposite>
                                      <feColorMatrix values="0 0 0 0 0.323977461   0 0 0 0 0.381127872   0 0 0 0 0.466853487  0 0 0 0.15 0" type="matrix" in="shadowBlurOuter1"></feColorMatrix>
                                  </filter>
                              </defs>
                              <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                  <g id="Menu" transform="translate(-32.000000, -56.000000)">
                                      <g id="Group-76">
                                          <g id="Group-45" transform="translate(0.000000, 48.000000)">
                                              <g id="Rectangle" opacity="0.600000024">
                                                  <use fill="black" fill-opacity="1" filter="url(#filter-2)" xlink:href="#path-1"></use>
                                                  <rect stroke="#FFFFFF" stroke-width="1" fill="#FFFFFF" fill-rule="evenodd" x="-0.5" y="-0.5" width="1217" height="73" rx="8"></rect>
                                              </g>
                                              <g id="SchoolBall_Logo_Rus-3" transform="translate(32.000000, 8.000000)">
                                                  <g id="SchoolBall_Logo_Rus-2">
                                                      <g id="SchoolBall_Logo_Rus">
                                                          <path d="M76.9129961,28.8800676 L3.06957198,28.8800676 C1.44607004,28.8800676 0.124980545,30.2190197 0.124980545,31.8651042 L0.124980545,44.6633915 C0.124980545,46.3086873 1.44607004,47.6477972 3.06957198,47.6477972 L76.9129961,47.6477972 C78.5368093,47.6477972 79.8578988,46.3086873 79.8578988,44.6633915 L79.8578988,31.8651042 C79.8578988,30.2190197 78.5368093,28.8800676 76.9129961,28.8800676" id="Fill-3" fill="#FFFFFE"></path>
                                                          <path d="M39.991284,0.0900732394 C24.7950195,0.0900732394 12.4322179,12.6203493 12.4322179,28.0225577 C12.4322179,43.4238197 24.7950195,55.9540958 39.991284,55.9540958 C55.1875486,55.9540958 67.5506615,43.4238197 67.5506615,28.0225577 C67.5506615,12.6203493 55.1875486,0.0900732394 39.991284,0.0900732394" id="Fill-4" fill="#FFFFFE"></path>
                                                          <path d="M25.4898054,13.8178028 C25.5634241,13.7376676 25.6406226,13.6594254 25.7184436,13.5807099 C28.6759533,10.5832113 32.4,8.62242254 36.4992996,7.89931268 L36.9148638,7.82580282 L36.6375097,8.14823662 C36.2084047,8.6484507 35.8720623,9.07468169 35.6381323,9.41494085 C35.5382101,9.56196056 35.4014008,9.76292958 35.3047471,9.97983099 L35.2729961,10.0516056 L35.1971984,10.0662761 C35.15393,10.0752676 30.8289494,10.9584901 26.9277821,14.281262 L26.8731518,14.3274817 L25.3067704,14.0173521 L25.4898054,13.8178028 Z M40.8779767,13.9766535 L40.9582879,14.0340732 L39.52607,20.9575662 L39.4552529,20.9891155 C35.5094163,22.7276394 33.0269261,24.3243493 32.1427237,24.9300958 L32.065214,24.9835718 L26.5909728,21.7887324 L26.5985992,21.6928225 C26.6641245,20.8805859 27.0138521,16.7534648 27.5021012,15.245093 L27.5150195,15.2053408 L27.5466148,15.1780507 C31.174786,11.9851042 35.6610117,11.1113465 35.7056809,11.1025127 L35.8387549,11.0952563 L35.8983658,11.1201803 C36.4037354,11.3626366 38.9827237,12.6198761 40.8779767,13.9766535 Z M42.0936965,14.1526986 C43.498677,13.629138 47.1842802,13.2753127 49.1402335,13.1486423 L49.1878599,13.1454873 L49.2284825,13.1710423 C52.8908949,15.4660958 54.8018677,18.817893 54.820856,18.8511775 L54.8470039,18.8980282 L54.8387549,18.9500845 C54.6731518,19.9968901 54.0747082,23.5651155 53.3077043,25.5514592 L53.2792218,25.6254423 L46.5797665,27.2590648 L40.5696498,21.168 L42.0158755,14.1809352 L42.0936965,14.1526986 Z M49.1047471,9.79021972 C51.0017121,10.7616225 52.7322957,12.0324282 54.2485603,13.5693521 C55.7802335,15.1211042 57.0459144,16.8962254 58.010428,18.8451831 L58.1794553,19.1867042 L55.7273152,18.2798197 L55.7000778,18.2336 C55.6790661,18.1979493 53.5637354,14.6451831 49.8311284,12.2783549 L49.789572,12.2523268 L48.7523735,9.60991549 L49.1047471,9.79021972 Z M20.5070817,33.2906592 C19.8263035,30.6968338 19.6544747,27.9804394 20.0152529,25.2739831 L20.0761089,24.821093 L21.0810895,26.6597859 L21.0761089,26.7078986 C20.825214,29.0188845 21.0954086,31.4554366 21.4205447,33.2906592 L22.5055253,33.2906592 C22.1715175,31.4715268 21.8835798,29.0272451 22.140856,26.7556958 L22.1598444,26.6970141 C22.8719066,25.455707 24.5634241,23.0145803 25.8692607,22.6612282 L25.9290272,22.6452958 L31.6233463,25.9668056 L33.1310506,33.2906592 L44.0555642,33.2906592 L46.7670039,28.3218028 L53.4561868,26.6924394 L53.5153307,26.7717859 C54.56,28.1758873 56.3905058,30.8705127 56.6205447,32.064969 L56.627393,32.099831 L56.6245914,32.1105577 C56.6227237,32.2326535 56.6040467,32.6416901 56.4490272,33.2906592 L57.5431907,33.2906592 C57.6038911,33.0007211 57.6396887,32.7588958 57.6602335,32.5711775 L57.6706615,32.4744789 L57.7612451,32.4435606 C58.2239689,32.2872338 58.9228016,31.9446085 59.6270817,31.2030423 L59.9596887,30.8533183 L59.8824903,31.3322366 C59.7758755,31.9955606 59.6347082,32.6481577 59.4672374,33.2906592 L60.5662257,33.2906592 C60.9433463,31.7643042 61.1626459,30.1882592 61.2032685,28.5819268 C61.2062257,28.5066817 61.2079377,28.4320676 61.2079377,28.3561915 C61.2521401,25.4850479 60.7453696,22.6855211 59.700856,20.0330141 L59.629572,19.8505014 C58.5657588,17.2115606 57.0084047,14.841893 55.0009339,12.8071211 C52.9953307,10.7747155 50.6578988,9.19677746 48.0521401,8.11747606 L47.9038132,8.05579718 C45.3845914,7.03454648 42.7256031,6.51587606 40.0015564,6.51587606 C39.4555642,6.51587606 38.9031907,6.53780282 38.3592218,6.58055211 L38.3511284,6.5811831 C38.3494163,6.5811831 38.3478599,6.5811831 38.3461479,6.5811831 L38.3142412,6.58402254 L38.3101946,6.58228732 C38.2518288,6.58528451 38.1943969,6.59049014 38.1397665,6.59711549 C33.1702724,7.03454648 28.4975875,9.23936901 24.9772763,12.8071211 C24.6073152,13.181769 24.2476265,13.5740845 23.9086381,13.9717634 C23.8775097,14.0086761 23.8627237,14.0257127 23.8488716,14.043538 C21.6714397,16.6212732 20.1157977,19.7254085 19.3450584,23.0257803 C19.3270039,23.095031 19.311751,23.1608113 19.2964981,23.2284845 L19.2930739,23.2504113 C18.5508171,26.5791775 18.6163424,30.0294085 19.4225681,33.2906592 L20.5070817,33.2906592 L20.5070817,33.2906592 Z" id="Fill-5" fill="#1A1919"></path>
                                                          <path d="M39.991284,1.15233803 C25.3729183,1.15233803 13.4801556,13.206062 13.4801556,28.0225577 C13.4801556,42.8382648 25.3729183,54.8921465 39.991284,54.8921465 C54.6096498,54.8921465 66.5025681,42.8382648 66.5025681,28.0225577 C66.5025681,13.206062 54.6096498,1.15233803 39.991284,1.15233803 Z M39.991284,2.21444507 C54.0541634,2.21444507 65.4546304,13.7684282 65.4546304,28.0225577 C65.4546304,42.2751099 54.0541634,53.8297239 39.991284,53.8297239 C25.9284047,53.8297239 14.528249,42.2751099 14.528249,28.0225577 C14.528249,13.7684282 25.9284047,2.21444507 39.991284,2.21444507 L39.991284,2.21444507 Z" id="Fill-6" fill="#1A1919"></path>
                                                          <path d="M3.06957198,46.0544 C2.31268482,46.0544 1.69696498,45.4301972 1.69696498,44.6633915 L1.69696498,31.8651042 C1.69696498,31.0975099 2.31268482,30.4734648 3.06957198,30.4734648 L76.9129961,30.4734648 C77.6700389,30.4734648 78.2859144,31.0975099 78.2859144,31.8651042 L78.2859144,44.6633915 C78.2859144,45.4301972 77.6700389,46.0544 76.9129961,46.0544 L3.06957198,46.0544" id="Fill-7" fill="#F0A936"></path>
                                                          <path d="M76.9129961,29.9423324 L3.06957198,29.9423324 C2.02381323,29.9423324 1.17291829,30.8047324 1.17291829,31.8651042 L1.17291829,44.6633915 C1.17291829,45.7231324 2.02381323,46.5855324 3.06957198,46.5855324 L76.9129961,46.5855324 C77.9590661,46.5855324 78.8099611,45.7231324 78.8099611,44.6633915 L78.8099611,31.8651042 C78.8099611,30.8047324 77.9590661,29.9423324 76.9129961,29.9423324 Z M76.9129961,31.0044394 C77.3800778,31.0044394 77.7618677,31.3915493 77.7618677,31.8651042 L77.7618677,44.6633915 C77.7618677,45.1363155 77.3800778,45.5234254 76.9129961,45.5234254 L3.06957198,45.5234254 C2.60280156,45.5234254 2.22101167,45.1363155 2.22101167,44.6633915 L2.22101167,31.8651042 C2.22101167,31.3915493 2.60280156,31.0044394 3.06957198,31.0044394 L76.9129961,31.0044394 L76.9129961,31.0044394 Z" id="Fill-8" fill="#1A1919"></path>
                                                          <polyline id="Fill-9" fill="#1A1919" points="6.14303502 41.5573634 6.14303502 34.9709746 7.86038911 34.9709746 7.86038911 40.005138 9.53182879 40.005138 9.53182879 34.9709746 11.248716 34.9709746 11.248716 40.005138 12.9203113 40.005138 12.9203113 34.9709746 14.637821 34.9709746 14.637821 41.5573634 6.14303502 41.5573634"></polyline>
                                                          <polyline id="Fill-10" fill="#1A1919" points="15.8607004 41.5573634 15.8607004 34.9709746 17.5780545 34.9709746 17.5780545 37.464631 18.3953307 37.464631 19.8711284 34.9709746 21.7743191 34.9709746 19.7321401 38.1787493 21.867393 41.5573634 19.9642023 41.5573634 18.3856809 39.0170141 17.5780545 39.0170141 17.5780545 41.5573634 15.8607004 41.5573634"></polyline>
                                                          <path d="M25.2449805,36.4291831 C24.1218677,36.4291831 23.5277821,37.2103437 23.5277821,38.2640901 C23.5277821,39.3175211 24.1218677,40.0986817 25.2449805,40.0986817 C26.368249,40.0986817 26.9623346,39.3175211 26.9623346,38.2640901 C26.9623346,37.2103437 26.368249,36.4291831 25.2449805,36.4291831 Z M25.2449805,34.8293183 C27.5662257,34.8293183 28.72607,36.4291831 28.72607,38.2640901 C28.72607,40.0986817 27.5662257,41.6985465 25.2449805,41.6985465 C22.9242023,41.6985465 21.7638911,40.0986817 21.7638911,38.2640901 C21.7638911,36.4291831 22.9242023,34.8293183 25.2449805,34.8293183 L25.2449805,34.8293183 Z" id="Fill-11" fill="#1A1919"></path>
                                                          <path d="M35.2700389,34.9709746 L35.2700389,41.5573634 L33.5520623,41.5573634 L33.5520623,36.5233577 L32.1505058,36.5233577 C31.9461479,39.2238197 31.621323,41.6327662 29.7554864,41.6420732 C29.51393,41.6420732 29.2445136,41.6042141 28.9565759,41.5105127 L28.9565759,39.9955155 C29.0776654,40.0329014 29.2261479,40.051831 29.402179,40.0329014 C30.4513619,39.9202704 30.4329961,36.9655211 30.5814786,34.9709746 L35.2700389,34.9709746" id="Fill-12" fill="#1A1919"></path>
                                                          <path d="M38.5357198,39.1107155 L40.2066926,39.1107155 L39.538677,37.2103437 C39.4828016,37.050231 39.4269261,36.8901183 39.3716732,36.5422873 C39.3154864,36.8901183 39.2596109,37.050231 39.204358,37.2103437 L38.5357198,39.1107155 Z M35.8896498,41.5573634 L38.4429572,34.9709746 L40.2997665,34.9709746 L42.8530739,41.5573634 L41.0893385,41.5573634 L40.7175097,40.5219155 L38.025214,40.5219155 L37.6540078,41.5573634 L35.8896498,41.5573634 L35.8896498,41.5573634 Z" id="Fill-13" fill="#1A1919"></path>
                                                          <polyline id="Fill-14" fill="#1A1919" points="53.2258366 34.9709746 53.2258366 41.5573634 51.5083268 41.5573634 51.5083268 38.0568113 50.1159533 40.2207775 49.1319844 40.2207775 47.7392996 38.0471887 47.7392996 41.5573634 46.0214786 41.5573634 46.0214786 34.9709746 47.6462257 34.9709746 49.6238132 38.0568113 51.6014008 34.9709746 53.2258366 34.9709746"></polyline>
                                                          <path d="M57.0113619,38.0004958 L58.0141634,38.0004958 L58.0141634,36.4758761 L57.0113619,36.4758761 C56.4264591,36.4758761 56.092607,36.7303211 56.092607,37.238107 C56.092607,37.7466817 56.4264591,38.0004958 57.0113619,38.0004958 Z M53.9203113,41.5573634 L55.6740856,39.1764958 C54.894786,38.8848225 54.3288716,38.2737127 54.3288716,37.238107 C54.3288716,35.5915493 55.5819455,34.9709746 56.9926848,34.9709746 L59.7321401,34.9709746 L59.7321401,41.5573634 L58.0141634,41.5573634 L58.0141634,39.4116958 L57.4107393,39.4116958 L55.8787549,41.5573634 L53.9203113,41.5573634 L53.9203113,41.5573634 Z" id="Fill-15" fill="#1A1919"></path>
                                                          <path d="M64.5388327,41.5573634 L64.5388327,39.3175211 C64.0096498,39.478107 63.4994553,39.5623437 63.0351751,39.5623437 C61.6238132,39.5623437 60.6675486,38.828507 60.6675486,37.2849577 L60.6675486,34.9709746 L62.3849027,34.9709746 L62.3849027,37.1914141 C62.3849027,37.7277521 62.8585214,37.991031 63.4715953,37.991031 C63.8054475,37.991031 64.1774319,37.9162592 64.5388327,37.765769 L64.5388327,34.9709746 L66.256965,34.9709746 L66.256965,41.5573634 L64.5388327,41.5573634" id="Fill-16" fill="#1A1919"></path>
                                                          <path d="M69.5232685,39.1107155 L71.1940856,39.1107155 L70.5256031,37.2103437 C70.4701946,37.050231 70.4138521,36.8901183 70.3584436,36.5422873 C70.3021012,36.8901183 70.2466926,37.050231 70.1911284,37.2103437 L69.5232685,39.1107155 Z M66.8770428,41.5573634 L69.4297276,34.9709746 L71.286537,34.9709746 L73.8398444,41.5573634 L72.0759533,41.5573634 L71.7045914,40.5219155 L69.0119844,40.5219155 L68.6414008,41.5573634 L66.8770428,41.5573634 L66.8770428,41.5573634 Z" id="Fill-17" fill="#1A1919"></path>
                                                          <polyline id="Fill-18" fill="#1A1919" points="39.7543969 3.96353803 39.7094163 4.05077183 39.6155642 4.06733521 39.0719066 4.15030986 39.4681712 4.54735775 39.5336965 4.61376901 39.5171984 4.70936338 39.4272374 5.26873239 39.9056809 5.00734648 39.9915953 4.96238873 40.0771984 5.00734648 40.555642 5.26873239 40.4655253 4.70936338 40.4494942 4.61376901 40.5147082 4.54735775 40.9111284 4.15030986 40.3673152 4.06733521 40.273463 4.05077183 40.2284825 3.96353803 39.9915953 3.46206197 39.7543969 3.96353803"></polyline>
                                                          <polyline id="Fill-19" fill="#1A1919" points="28.8933852 6.75328451 28.8929183 6.8520338 28.8168093 6.91008451 28.3718288 7.23661972 28.9033463 7.4055662 28.9915953 7.43396056 29.0203891 7.52655775 29.193463 8.06526197 29.5005447 7.61063662 29.5559533 7.53065915 29.6530739 7.53113239 30.1964202 7.54075493 29.863035 7.08549859 29.8057588 7.00851831 29.8333074 6.91891831 30.0057588 6.38131831 29.4852918 6.56036056 29.3945525 6.58938592 29.3151751 6.53307042 28.8776654 6.19754366 28.8933852 6.75328451"></polyline>
                                                          <polyline id="Fill-20" fill="#1A1919" points="20.5050584 14.281262 20.5492607 14.368969 20.5080156 14.4562028 20.2597665 14.9534197 20.8088716 14.8562479 20.9000778 14.8407887 20.9676265 14.9092507 21.3651362 15.3078761 21.4323735 14.7612845 21.4457588 14.6641127 21.5316732 14.6197859 22.0188327 14.3754366 21.5168872 14.1258817 21.4308171 14.0837634 21.4150973 13.9913239 21.3252918 13.4335324 20.9433463 13.8348394 20.8761089 13.9031437 20.7796109 13.8894197 20.2392218 13.7950873 20.5050584 14.281262"></polyline>
                                                          <polyline id="Fill-21" fill="#1A1919" points="16.4572763 24.8703099 16.5361868 24.9277296 16.5388327 25.0237972 16.5431907 25.5815887 16.9870817 25.2395944 17.061323 25.1834366 17.1523735 25.2129352 17.6859144 25.3823549 17.4985214 24.8655775 17.4663035 24.773138 17.5224903 24.6936338 17.8449805 24.2500507 17.2859144 24.2615662 17.1906615 24.2645634 17.134786 24.1899493 16.8026459 23.7357972 16.6446693 24.2699268 16.6157198 24.3615775 16.5238911 24.3943887 16.0007782 24.5614423 16.4572763 24.8703099"></polyline>
                                                          <polyline id="Fill-22" fill="#1A1919" points="51.1055253 6.19754366 50.6680156 6.53307042 50.5884825 6.58938592 50.4978988 6.56036056 49.9768093 6.38131831 50.1492607 6.91891831 50.1774319 7.00851831 50.12 7.08549859 49.7867704 7.54075493 50.3299611 7.53113239 50.4264591 7.53065915 50.4828016 7.61063662 50.7897276 8.06526197 50.9623346 7.52655775 50.9914397 7.43396056 51.0792218 7.4055662 51.6113619 7.23661972 51.1657588 6.91008451 51.0896498 6.8520338 51.0891829 6.75328451 51.1055253 6.19754366"></polyline>
                                                          <polyline id="Fill-23" fill="#1A1919" points="59.7436576 13.7950873 59.2032685 13.8894197 59.1067704 13.9031437 59.0393774 13.8348394 58.6575875 13.4335324 58.5677821 13.9913239 58.5519066 14.0837634 58.4661479 14.1258817 57.9640467 14.3754366 58.4512062 14.6197859 58.5371206 14.6641127 58.5506615 14.7612845 58.6177432 15.3078761 59.0152529 14.9092507 59.0826459 14.8407887 59.1738521 14.8562479 59.7231128 14.9534197 59.4748638 14.4562028 59.4336187 14.368969 59.477821 14.281262 59.7436576 13.7950873"></polyline>
                                                          <polyline id="Fill-24" fill="#1A1919" points="63.9821012 24.5614423 63.4589883 24.3943887 63.3671595 24.3615775 63.3380545 24.2699268 63.1803891 23.7357972 62.8480934 24.1899493 62.7922179 24.2645634 62.696965 24.2615662 62.1377432 24.2500507 62.4603891 24.6936338 62.5165759 24.773138 62.484358 24.8655775 62.296965 25.3823549 62.8306615 25.2129352 62.9215564 25.1834366 62.9957977 25.2395944 63.4396887 25.5815887 63.4440467 25.0237972 63.4466926 24.9277296 63.5256031 24.8703099 63.9821012 24.5614423"></polyline>
                                                          <path d="M33.7590661,50.1988732 C34.1280934,49.8556169 34.3410117,49.5669408 34.3410117,49.1086873 C34.3410117,48.4792789 33.8994553,48.0886986 33.1877043,48.0886986 C32.8578988,48.0886986 32.5262257,48.1656789 32.2789105,48.2991324 L32.2099611,48.3363606 L32.4456031,48.9552 L32.5344747,48.9037746 C32.6941634,48.8111775 32.913463,48.754231 33.1067704,48.754231 C33.3316732,48.754231 33.5993774,48.8141746 33.5993774,49.1004845 C33.5993774,49.3304789 33.5064591,49.4987944 33.2319066,49.7666479 C32.8871595,50.0998085 32.5856809,50.3515718 32.3101946,50.5378704 L32.2708171,50.5645296 L32.2708171,51.0623775 L34.3878599,51.0623775 L34.3878599,50.4050479 L33.5123735,50.4050479 C33.5929961,50.3444732 33.6754864,50.2771155 33.7590661,50.1988732" id="Fill-25" fill="#1A1919"></path>
                                                          <path d="M37.6916732,50.444169 C37.2287938,50.444169 36.892607,50.0890817 36.892607,49.5995944 C36.892607,49.1099493 37.2287938,48.754231 37.6916732,48.754231 C38.1547082,48.754231 38.4902724,49.1099493 38.4902724,49.5995944 C38.4902724,50.0890817 38.1547082,50.444169 37.6916732,50.444169 Z M37.6916732,48.0886986 C36.8136965,48.0886986 36.1511284,48.7382986 36.1511284,49.5995944 C36.1511284,50.4602592 36.8136965,51.1098592 37.6916732,51.1098592 C38.5699611,51.1098592 39.2323735,50.4602592 39.2323735,49.5995944 C39.2323735,48.7382986 38.5699611,48.0886986 37.6916732,48.0886986 L37.6916732,48.0886986 Z" id="Fill-26" fill="#1A1919"></path>
                                                          <path d="M42.4210117,50.444169 C41.9582879,50.444169 41.6224125,50.0890817 41.6224125,49.5995944 C41.6224125,49.1099493 41.9582879,48.754231 42.4210117,48.754231 C42.8837354,48.754231 43.2199222,49.1099493 43.2199222,49.5995944 C43.2199222,50.0890817 42.8837354,50.444169 42.4210117,50.444169 Z M42.4210117,48.0886986 C41.5424125,48.0886986 40.8804669,48.7382986 40.8804669,49.5995944 C40.8804669,50.4602592 41.5424125,51.1098592 42.4210117,51.1098592 C43.298677,51.1098592 43.9612451,50.4602592 43.9612451,49.5995944 C43.9612451,48.7382986 43.298677,48.0886986 42.4210117,48.0886986 L42.4210117,48.0886986 Z" id="Fill-27" fill="#1A1919"></path>
                                                          <polyline id="Fill-28" fill="#1A1919" points="45.6678599 48.1360225 45.6678599 48.7933521 46.9361868 48.7933521 45.9906615 51.0623775 46.7833463 51.0623775 47.76607 48.620462 47.772607 48.1360225 45.6678599 48.1360225"></polyline>
                                                          <path d="M74.8412451,32.6517859 C74.1543969,32.6517859 73.4592996,33.1103549 73.4592996,33.9869521 C73.4592996,34.863707 74.1543969,35.3222761 74.8412451,35.3222761 C75.5262257,35.3222761 76.2194553,34.863707 76.2194553,33.9869521 C76.2194553,33.1103549 75.5262257,32.6517859 74.8412451,32.6517859 Z M74.8412451,35.637769 C73.9998444,35.637769 73.1480156,35.0706704 73.1480156,33.9869521 C73.1480156,32.9033915 73.9998444,32.336293 74.8412451,32.336293 C75.6807782,32.336293 76.5307393,32.9033915 76.5307393,33.9869521 C76.5307393,35.0706704 75.6807782,35.637769 74.8412451,35.637769 L74.8412451,35.637769 Z" id="Fill-29" fill="#1A1919"></path>
                                                          <path d="M74.9263813,33.925431 L74.5985992,33.925431 L74.5985992,33.5357972 L74.9263813,33.5357972 C75.0396887,33.5357972 75.193463,33.556462 75.193463,33.7326648 C75.193463,33.9050817 75.0396887,33.925431 74.9263813,33.925431 Z M75.2703502,34.2043268 C75.4566537,34.1470648 75.6063813,33.9869521 75.6063813,33.7162592 C75.6063813,33.3143211 75.2703502,33.1666704 74.9749416,33.1666704 L74.2101167,33.1666704 L74.2101167,34.7704789 L74.5985992,34.7704789 L74.5985992,34.2454986 L74.8333074,34.2454986 C74.8454475,34.2454986 74.8575875,34.2454986 74.8697276,34.2454986 L75.2582101,34.7704789 L75.6992996,34.7704789 L75.2703502,34.2043268 L75.2703502,34.2043268 Z" id="Fill-30" fill="#1A1919"></path>
                                                      </g>
                                                  </g>
                                              </g>
                                          </g>
                                      </g>
                                  </g>
                              </g>
                            </div>
                          </div>
                        <div class="col-md-12">
                          <h4>Ваша заявка успешно отправлена! <br> В ближайшее время с вами свяжется менеджер!</h4>
                        </div>
                      </div>
                    </div>
                  </div>
              </div><!-- /.modal-content -->
          </div><!-- /.modal-dialog -->

*/ ?>




    <!-- Modal Contact form -->
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog ">
        <div class="modal-content">
            <div class="modal-header text-left">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="color: #343a40;width: unset;height: unset;background: unset;border: unset;font-size: 1.5rem;line-height: 1;">×</button>
            </div>
            <div id="callback-1-front">
                <div class="modal-body text-left">
                    <h5 class="modal-title2 mt-0"  id="myLargeModalLabel">Заполните форму, чтобы получить консультацию

                    </h5>

                    <p style="
    text-align: left;
    margin: 16px 3px 24px 7px;
    height: AUTO;
">
                        Забронируйте место в смене со скидкой, а мы перезвоним, проконсультируем и оформим заказ.
                    </p>

                    <div class="">
                        <div class="col-md-12">
                        </div>
                        <form id="sendform" method="post" class="needs-validation" novalidate>
                            @csrf
                            <div class="row">
                                <div class="col-md-12 col-xl-12  ">
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control" id="name" name="name" placeholder="name@example.com" required>
                                        <label for="name">Имя</label>
                                        <div class="invalid-feedback">
                                            Поле обязательно для заполнения
                                        </div>
                                    </div>
                                </div>

                            </div>
                            <div class="row">
                                <div class="col-md-12 col-xl-12">
                                    <div class="form-floating mb-3">
                                        <input type="text" class="form-control phone" id="phone" name="phone" placeholder="name@example.com" required>
                                        <label for="phone">Мобильный телефон</label>
                                        <div class="invalid-feedback">
                                            Поле обязательно для заполнения
                                        </div>
                                    </div>
                                </div>
                            </div>






                            <div class="row">
                                <div class="col-md-6 col-xl-6">
                                    <button class="btn btn-primary" style="margin-top: 24px !important;" type="submit" onclick="sendForm(event)">Отправить заявку</button>
                                </div>
                            </div>

                        </form>

                        <div class="sogl text-left" style="
    margin-top: 16px;
    margin-bottom: 56px;
">
                            <label id="sogl-1"for="sogl">Нажимая на кнопку, вы даёте согласие на <a href="{{ asset('assets/doc/sogl-ooo_sm.pdf')}}" target='_blank'>обработку персональных данных.</a></label>
                        </div>

                    </div>


                </div>
            </div>
            <div id="callback-1-done" class="d-none">
                <div class="modal-body text-center" style="margin: 32px;">
                    <div  >
                        <div class="col-md-12">
                            <h3 style="font-size: 48px;">  🥳    </h3>                   </div>
                        <div class="col-md-12">
                            <h4> <b> Спасибо за заявку </b> </h4>

                            <p class="psy-about">Наш менеджер свяжется с вами в ближайшее время. А пока почитайте наш блог по воспитанию настоящих лидеров и чемпионов! </p><br>

                            <a href="{{ route('blog') }}" class="btn btn-primary" style="    margin-top: 0px !important;
    margin-bottom: 56px !important;
    width: 100% !important;">Читать блог</a>


                        </div>
                    </div>
                </div>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->

</div>

<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header text-center">
                <h5 class="modal-title mt-0" id="myLargeModalLabel">Записаться на тренировку</h5>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="color: #343a40;width: unset;height: unset;background: unset;border: unset;font-size: 1.5rem;line-height: 1;">×</button>
            </div>
            <div id="callback-1-front">
                <div class="modal-body text-center">
                    <div class="row">
                        <div class="col-md-12">

 <img src="{{ asset('assets/sb-ui/icons/logo-sb.svg')}}" class="mb-5"   width="200px" style="width:200px;">


                    <form id="sendform" method="post">
                        @csrf
                        <div class="row">
                            <label for="name" class="col-md-4 col-form-label-new text-md-right">Как к Вам обращаться? <span style="text-align: left;color: red;font-size:20px;">*</span></label>

                            <div class="col-md-6">
                                <input type="text" name="name" value="" required class="form-control-new" placeholder="Петров Пётр">
                            </div>

                        </div>
                        <div class="row">
                            <label for="phone" class="col-md-4 col-form-label-new text-md-right">Ваш контактный телефон: <span style="text-align: left;color: red;font-size:20px;">*</span></label>
                            <div class="col-md-6">
                                <input type="text" name="phone" value="" required class="phone form-control-new" >
                            </div>
                        </div>

                        <div class="row">
                            <label for="phone" class="col-md-4 col-form-label-new text-md-right">Ваш email:</label>
                            <div class="col-md-6">
                                <input type="text" name="email" value="" required class="email form-control-new" >
                            </div>
                        </div>
                        <div class="row">
                            <div class=""><p style="color: red;
    height: auto;
    margin: 16px;
    font-size: 14px;
    text-align: left !important;">* Поля для обязательного заполнения</p></div></div>
                        <hr>



                    </form>
                    <button id="sogl-btn text-center" type="submit" class="btn btn-primary waves-effect waves-light" style="margin: 0 auto; color: #54420a; background-color: #fede11; border: solid 1px #fede11; box-shadow: none;" onclick="sendForm()" >Отправить заявку</button>
                    <br><br>
                    <div class="sogl text-center">
                        <label id="sogl-1"for="sogl">Я согласен на <a href="{{ asset('assets/doc/sogl-ooo_sm.pdf')}}" target='_blank'>обработку персональных данных</a></label>
                    </div>
                        </div>
                </div>


            </div>



    </div><!-- /.modal-content -->

            <div id="callback-1-done" class="d-none">
                <div class="modal-body text-center">
                    <div class="col-md-12">
                        <h4>Ваша заявка успешно отправлена! <br> В ближайшее время с вами свяжется менеджер!</h4>
                    </div>
                </div>
            </div>
        </div>

    </div><!-- /.modal-dialog -->
</div>


<div class="modal fade bs-example-modal-lg2" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog ">
        <div class="modal-content">

            <div id="sendpsychologist-1-front" class="psyfront">
                <div class="modal-header text-left">
                     <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="color: #343a40;width: unset;height: unset;background: unset;border: unset;font-size: 1.5rem;line-height: 1;">×</button>
                </div>

                <h5 class="modal-title2 mt-0"  id="myLargeModalLabel">Введите данные, чтобы скачать рекомендации</h5>

                <div class="modal-body text-center">
                    <div class="row">
                        <div class="col-md-12">
                     </div>
                    <form id="sendpsychologist" method="post">
                        @csrf



                        <div class="row">
                            <label for="phone" class=" col-form-label-new text-md-right">Мобильный телефон: <span style="text-align: left;color: red;font-size:20px;">*</span></label><br>
                        </div>
                            <div class="row">
                                <input type="text" name="phone" value="" required class="phone form-control-new2" >
                        </div>
                        <div class="row">
                            <label for="phone" class="  col-form-label-new text-md-right">Электронная почта:</label>
                        </div>
                        <div class="row">
                                <input type="text" name="email" value="" required class="email form-control-new2" >
                        </div>


                    </form>
                        <div class="row">
                            <button id="sogl-btn text-center" type="submit" class="btn btn-primary-modal  waves-effect waves-light  mt-4" style="
                               font-family: 'IBMPlexSans-Medium';
    font-size: 18px;
    font-weight: 500;
    font-stretch: normal;
    font-style: normal;
    line-height: 1.33;
    letter-spacing: normal;
    text-align: center;
    color: #54420a;" onclick="sendPsychologist()" >Скачать</button>
                        </div>
                     <br><br>
                        <div class="row">
                    <div class="sogl2 text-left">
                        <label id="sogl-1"for="sogl">

                            Нажимая на кнопку, вы даёте согласие
                            на <a href="{{ asset('assets/doc/sogl-ooo_sm.pdf')}}" target='_blank'>обработку персональных данных</a>.

                        </label>
                    </div>
                        </div>
                </div>
                </div>

            </div>
            <div id="sendpsychologist-1-done" class="d-none">
                <div class="modal-body text-center" style="margin: 47px;">
                    <div class="row">
                        <div class="col-md-12">
                          <h3 style="font-size: 48px;">  🥳    </h3>                   </div>
                    <div class="col-md-12">
                        <h4> <b> Готово!  </b> </h4>
                        <br>
                        <p class="psy-about">Рекомендации от нашего психолого уже скачиваются на ваше устройство. </p>
                        <button type="button" class="btn btn-modal text-center mt-3" data-dismiss="modal" aria-hidden="true">
                            Хорошо</button>

                    </div>
                </div>
            </div>

    </div><!-- /.modal-content -->
</div><!-- /.modal-dialog -->

    </div>
</div>



<!-- Modal Contact form -->
<script>
    function sendFormCatalog(event) {
        event.preventDefault();
        var form = $("#send_form_catalog")[0];
        if (form.checkValidity()) {
            var currentPage = window.location.pathname.split('/').pop();
            $("#send_form_catalog").append('<input type="hidden" name="path" value="'+currentPage+'">');
            $.ajax({
                type: "POST",
                url: "{{ route('backCall2') }}",
                data: $("#send_form_catalog").serialize(),
                success: function( result ) {
                    if (result == 'DONE!') {
                        $("#callback-catalog-front").addClass("d-none");
                        $("#callback-catalog-done").removeClass("d-none");
                    }
                }
            });
        }
        form.classList.add('was-validated');
    }
</script>

<!-- /.modal -->
<script>
    (function() {
        'use strict';
        window.addEventListener('load', function() {
            var forms = document.getElementsByClassName('needs-validation');
            var validation = Array.prototype.filter.call(forms, function(form) {
                form.addEventListener('submit', function(event) {
                    if (form.checkValidity() === false) {
                        event.preventDefault();
                        event.stopPropagation();
                    }
                    form.classList.add('needs-validation');
                }, false);
            });
        }, false);
    })();
</script>

<script>
    function sendForm(event) {
        event.preventDefault();
        var form = $("#sendform")[0];
        if (form.checkValidity()) {
            var currentPage = window.location.pathname.split('/').pop();
            $("#sendform").append('<input type="hidden" name="path" value="'+currentPage+'">');
            $.ajax({
                type: "POST",
                url: "{{ route('backCall2') }}",
                data: $("#sendform").serialize(),
                success: function( result ) {
                    if (result == 'DONE!') {
                        $("#callback-1-front").addClass("d-none");
                        $("#callback-1-done").removeClass("d-none");
                    }
                }
            });
        }
        form.classList.add('was-validated');
    }



    var currentProgramId;

    function setProgramId(programId) {
        currentProgramId = programId;
    }

    function sendProgram(event) {
        event.preventDefault();
        var form = $("#sendprogram")[0];
        if (form.checkValidity()) {
            var currentPage = window.location.pathname.split('/').pop();
            $("#sendprogram").append('<input type="hidden" name="path" value="' + currentPage + '">');
            $("#sendprogram").append('<input type="hidden" name="program_id" value="' + currentProgramId + '">');
            $.ajax({
                type: "POST",
                url: "{{ route('backCall4') }}",
                data: $("#sendprogram").serialize(),
                success: function(result) {
                    window.open(result.pdf_url, '_blank');
                    $("#sendprogram-1-front").addClass("d-none");
                    $("#sendprogram-1-done").removeClass("d-none");
                }
            });
        }
        form.classList.add('was-validated');
    }

    <?php /*
    function sendProgram() {
        if ($("#sendprogram")[0][1].value !== '' && $("#sendprogram")[0][2].value !== '') {
            // get current page name
            var currentPage = window.location.pathname.split('/').pop();
            // add current page name to form data
            $("#sendprogram").append('<input type="hidden" name="path" value="'+currentPage+'">');

            // add program id to form data
            $("#sendprogram").append('<input type="hidden" name="program_id" value="'+currentProgramId+'">');

            $.ajax({
                type: "POST",
                url: "{{ route('backCall4') }}",
                data: $("#sendprogram").serialize(),
                success: function( result ) {
                    // Open the PDF in a new window using the returned URL
                    window.open(result.pdf_url, '_blank');
                    $("#sendprogram-1-front").addClass("d-none");
                    $("#sendprogram-1-done").removeClass("d-none");
                }
            });

        } else {
            alert('Не введены данные в поля обязательные для заполнения')
        }
    }
 */ ?>




    function sendPsychologist() {
        if ($("#sendpsychologist")[0][1].value !== '' && $("#sendpsychologist")[0][2].value !== '') {
            // get current page name
            var currentPage = window.location.pathname.split('/').pop();
            // add current page name to form data
            $("#sendpsychologist").append('<input type="hidden" name="path" value="'+currentPage+'">');
            $.ajax({
                type: "POST",
                url: "{{ route('backCall3') }}",
                data: $("#sendpsychologist").serialize(),
                success: function( result ) {
                    if (result == 'DONE!') {
                        var link = document.createElement('a');
                        link.setAttribute('href', '{{ asset('downloads/PDF_1.pdf') }}');
                        link.setAttribute('download', 'PDF_1.pdf');
                        link.click();
                        $("#sendpsychologist-1-front").addClass("d-none");
                        $("#sendpsychologist-1-done").removeClass("d-none");
                    }
                }
            });

        } else {
            alert('Не введены данные в поля обязательные для заполнения')
        }
    }

    function sendFormPhone() {
        if ($("#sendformphone")[0][1].value !== '') {
            $.ajax({
                type: "POST",
                url: "{{ route('backCallPhone') }}",
                data: $("#sendformphone").serialize(),
                success: function( result ) {
                    if (result == 'DONE!') {
                        $("#callback-phone1-front").addClass("d-none");
                        $("#callback-phone1-done").removeClass("d-none");
                    }
                }
            });

        } else {
            alert('Не введены данные в поля обязательные для заполнения')
        }

    }
</script>

<script>
    function time_select(num) {
        let smenaSelect = document.getElementById('smenaSelect')
        let smenaSelect_place = document.getElementById('smenaSelect_place')
        while (smenaSelect.childNodes.length > 0) {
            smenaSelect.firstChild.remove();
        }
        if (num == 1) {
            var arr = [['0', 'Не выбрано'], ['1', '1 смена (30 мая - 10 июня)'], ['2', '2 смена (13 июня - 24 июня)'], ['3', '3 смена (27 июня - 8 июля)'], ['4', '4 смена (11 июля - 22 июля)'], ['5', '5 смена (25 июля - 5 августа)'], ['6', '6 смена (8 августа - 19 августа)'], ['7', '7 смена (22 августа - 26 августа)']]
        } else if (num == 2) {
            var arr = [['0', 'Не выбрано'], ['1', '1 смена (6 июня – 17 июня)'], ['2', '2 смена (20 июня - 1 июля)'], ['3', '3 смена (4 июля - 15 июля)'], ['4', '4 смена (18 июля - 29 июля)'], ['5', '5 смена (1 августа - 12 августа)'], ['6', '6 смена (15 августа - 26 августа)']]
        } else if (num == 3) {
            var arr = [['0', 'Не выбрано'], ['1', '1 смена (17 июня – 26 июня)'], ['2', '2 смена (5 августа – 14 август)']]
        }
        for (e in arr) {
            var opt = document.createElement('option');
            opt.value = arr[e][0];
            opt.text = arr[e][1];
            smenaSelect.appendChild(opt);
        }
        smenaSelect_place.setAttribute('style','visibility:visible');

        if (num == 1) {
            document.getElementById('sogl-3').classList.add("d-none");
            document.getElementById('sogl-2').classList.add("d-none");
            document.getElementById('sogl-1').classList.remove("d-none");
        } else if (num == 2){
            document.getElementById('sogl-3').classList.add("d-none");
            document.getElementById('sogl-1').classList.add("d-none");
            document.getElementById('sogl-2').classList.remove("d-none");
        } else if (num == 3) {
            document.getElementById('sogl-2').classList.add("d-none");
            document.getElementById('sogl-1').classList.add("d-none");
            document.getElementById('sogl-3').classList.remove("d-none");
        } else {
            document.getElementById('sogl-2').classList.add("d-none");
            document.getElementById('sogl-1').classList.add("d-none");
            document.getElementById('sogl-3').classList.add("d-none");
        }
        document.getElementById('submitarea').classList.remove("d-none");
    }

    function checkButton() {
        let ch = document.getElementById('sogl').checked;
        let btn = document.getElementById('sogl-btn');
        if (ch == true) {
            btn.removeAttribute('disabled');
        } else {
            btn.setAttribute('disabled', 'true');
        }
    }
</script>



<?php /*
    <script>
        function sendForm() {
          $.ajax({
              type: "POST",
              url: "{{ route('backCall', ['id' => 1]) }}",
              data: $("#sendform").serialize(),
              success: function( result ) {
                if (result == 'DONE!') {
                  $("#callback-1-front").addClass("d-none");
                  $("#callback-1-done").removeClass("d-none");
                }
              }
          });
        }
      </script>
*/

?>

<form id="methods" action="{{ route('methods', ['id' => 1]) }}" method="GET" style="display: none;">
</form>
<form id="clubslist" action="{{ route('clubs') }}" method="GET" style="display: none;">
</form>
<form id="championats" action="{{ route('championats') }}" method="GET" style="display: none;">
</form>
<form id="news" action="{{ route('news') }}" method="GET" style="display: none;">
</form>
<form id="faq" action="{{ route('faq') }}" method="GET" style="display: none;">
</form>

<!-- Optional JavaScript -->
<script src="{{ asset('js/vue.min.js') }}"></script>
<script src="{{ asset('js/datepicker.min.js') }}"></script>






<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.0.3/jquery.min.js"></script>
<script src="https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/jquery-ui.min.js"></script>

<script src="https://rawgit.com/Artemeey/5ebc39370e568c34f03dce1639cabee8/raw/8de40b26479c406ee9cd6f9b4b3f4ad05370a024/jquery.datepicker.extension.range.min.js"></script>
<script src="{{ asset('js/jquery.mask.min.js') }}"></script>
<!-- jQuery first, then Popper.js, then Bootstrap JS -->
<!-- <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js" integrity="sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj" crossorigin="anonymous"></script> -->
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
<script src="{{ asset('css/bootstrap-4.5.0-dist/js/bootstrap.min.js') }}"></script>
<script src="https://kit.fontawesome.com/32beb060e6.js" crossorigin="anonymous"></script>


<script src="{{ asset('js/slick/slick.js') }}" type="text/javascript" charset="utf-8"></script>
<script src="{{ asset('assets/plugins/tippy/tippy.all.min.js') }}"></script>

<script src="{{ asset('js/script.js') }}"></script>
<script src="{{ asset('js/jquery.nivo.slider.js') }}"></script>

<script src="{{ asset('js/app-scroll.js') }}"></script>
<script src="{{ asset('js/app-scroll-mob.js') }}"></script>
        <!-- JS mob slider -->
        <script src="{{ asset('js/vendors-slider.js') }}"></script>
        <script src="{{ asset('js/home-slider.js') }}"></script>
        <script src="{{ asset('js/app-slider.js') }}"></script>


        <script>

            !function(e) {
                function t(t) {
                    for(var o, r, i=t[0], c=t[1], l=t[2], d=0, p=[]; d<i.length; d++)r=i[d],
                    Object.prototype.hasOwnProperty.call(s, r)&&s[r]&&p.push(s[r][0]),
                        s[r]=0;
                    for(o in c)Object.prototype.hasOwnProperty.call(c, o)&&(e[o]=c[o]);
                    for(u&&u(t); p.length; )p.shift()();
                    return a.push.apply(a, l||[]),
                        n()
                }

                function n() {
                    for(var e, t=0; t<a.length; t++) {
                        for(var n=a[t], o= !0, i=1; i<n.length; i++) {
                            var c=n[i];
                            0 !==s[c]&&(o= !1)
                        }

                        o&&(a.splice(t--, 1), e=r(r.s=n[0]))
                    }

                    return e
                }

                var o= {}

                    ,
                    s= {
                        2: 0
                    }

                    ,
                    a=[];

                function r(t) {
                    if(o[t])return o[t].exports;

                    var n=o[t]= {

                            i:t,
                            l: !1,
                            exports: {}
                        }

                    ;
                    return e[t].call(n.exports, n, n.exports, r),
                        n.l= !0,
                        n.exports
                }

                r.m=e,
                    r.c=o,
                    r.d=function(e, t, n) {
                        r.o(e, t)||Object.defineProperty(e, t, {
                            enumerable: !0, get:n
                        })
                    }

                    ,
                    r.r=function(e) {
                        "undefined" !=typeof Symbol&&Symbol.toStringTag&&Object.defineProperty(e, Symbol.toStringTag, {
                            value:"Module"

                        }),
                            Object.defineProperty(e, "__esModule", {
                                value: !0
                            })
                    }

                    ,
                    r.t=function(e, t) {
                        if(1&t&&(e=r(e)), 8&t)return e;
                        if(4&t&&"object"==typeof e&&e&&e.__esModule)return e;
                        var n=Object.create(null);

                        if(r.r(n), Object.defineProperty(n, "default", {
                            enumerable: !0, value:e

                        }), 2&t&&"string" !=typeof e)for(var o in e)r.d(n, o, function(t) {
                            return e[t]
                        }

                            .bind(null, o));
                        return n
                    }

                    ,
                    r.n=function(e) {
                        var t=e&&e.__esModule?function() {
                                    return e.default
                                }

                                :function() {
                                    return e
                                }

                        ;
                        return r.d(t, "a", t),
                            t
                    }

                    ,
                    r.o=function(e, t) {
                        return Object.prototype.hasOwnProperty.call(e, t)
                    }

                    ,
                    r.p="public/js/";
                var i=window.webpackJsonp=window.webpackJsonp||[],
                    c=i.push.bind(i);
                i.push=t,
                    i=i.slice();
                for(var l=0; l<i.length; l++)t(i[l]);
                var u=c;
                a.push([6, 0]),
                    n()
            }

            ({
                1:function(e, t, n) {
                    "use strict"; var o=n(2), s=n.n(o); t.a=()=> {
                        if( !1===(()=> {
                            const e=document.createElement("canvas"); return !( !e.getContext|| !e.getContext("2d"))&&0===e.toDataURL("image/webp").indexOf("data:image/webp")

                        })()) {
                            document.querySelectorAll(".lazy").forEach(e=> {
                                if(e.hasAttribute("data-srcset")) {
                                    const t=e.getAttribute("data-srcset").split("."); "gif" !==t[1]&&e.setAttribute("data-srcset", t[0]+".webp")
                                }

                                if(e.hasAttribute("data-background-image")) {
                                    const t=e.getAttribute("data-background-image").split("."); "gif" !==t[1]&&e.setAttribute("data-background-image", t[0]+".webp")
                                }
                            })
                        }

                        const e=document.querySelectorAll(".lazy"); s()(e).observe()
                    }
                }

                , 6:function(e, t, n) {
                    e.exports=n(8)
                }

                , 8:function(e, t, n) {
                    "use strict"; n.r(t); var o=n(0), s=n.n(o), a=n(3), r=n.n(a), i=()=> {
                            const e=s()(".offer__overlay"), t=s()(".headerWrap"), n=s()(".header"); s()(window).on("scroll", (function() {
                                const o=s()(this).scrollTop(), a=1-(o-e.outerHeight()/2+500)/500; e.css({
                                    opacity:a

                                }), a>"1" ?e.css({
                                    opacity:1

                                }):a<"0" &&e.css({
                                    opacity:0

                                }), o>700?(t.addClass("headerWrap_afterScroll"), t.css({
                                        top:0
                                    }

                                    , 2e3), n.addClass("header_afterScroll")):(t.removeClass("headerWrap_afterScroll"), n.removeClass("header_afterScroll"))
                            }))
                        }

                        , c=(n(7), e=> {
                            s()("." +e).on("init", (function() {
                                s()(`.${e} [data-slick-index="1"]`).addClass("slick-next-slide")

                            })), s()("." +e).on("beforeChange", (function(t, n, o, a) {
                                s()(`.${e} .slick-slide`).removeClass("slick-prev-slide").removeClass("slick-next-slide"), s()(`.${e} [data-slick-index="${a-1}"]`).toggleClass("slick-prev-slide"), s()(`.${e} [data-slick-index="${a+1}"]`).toggleClass("slick-next-slide")

                            })), s()("." +e).slick({
                                centerMode: !0, arrows: !0, variableWidth: !0, infinite: !1, centerPadding:"0px", slidesToShow:1, nextArrow:`#${e}-next`, prevArrow:`#${e}-prev`
                            })

                        }), l=()=> {
                            s()(".tabs").each((function(e, t) {
                                const n=s()(t).children(".tabs__head").find(".tabs__nav-item"), o=s()(t).children(".tabs__container"), a=s()(t).children(".tabs__container").children(".tabs__item"); n.each((function(e, t) {
                                    s()(t).click((function() {
                                        n.each((function(e, t) {
                                            s()(t).removeClass("tabs__nav-item_active")

                                        })), a.each((function(e, t) {
                                            s()(t).removeClass("tabs__item_active")
                                        })), s()(t).addClass("tabs__nav-item_active"), o.children().eq(e).addClass("tabs__item_active")
                                    }))
                                }))
                            }))
                        }

                        , u=()=> {
                            let e=(e, t, n)=> {
                                    t.offsetHeight>n?(e.classList.add("tabs__btn-wrapper_visible"), t.classList.add("tabs__head_left")):(e.classList.remove("tabs__btn-wrapper_visible"), t.classList.remove("tabs__head_left"))
                                }

                            ; document.querySelectorAll(".tabs").forEach((function(t, n) {
                                let o=t.querySelector(".tabs__head"), s=t.querySelector(".tabs__nav"), a=t.querySelector(".tabs__btn-wrapper"), r=t.querySelector(".tabs__btn"), i=o.offsetHeight; t.querySelectorAll(".tabs__nav-item").forEach((function(e, t, n) {
                                    e.onclick=()=> {
                                        setTimeout((function() {}), 300)
                                    }

                                })), window.addEventListener("resize", (function(t) {
                                    e(a, s, i), r.classList.contains("tabs__btn_active")&&(o.style.height=s.offsetHeight+"px")

                                })), a.onclick=()=> {
                                    r.classList.contains("tabs__btn_active")?(r.classList.remove("tabs__btn_active"), o.style.height=i+"px"):(r.classList.add("tabs__btn_active"), o.style.height=s.offsetHeight+"px")
                                }

                                    , e(a, s, i)
                            }))
                        }

                        , d=()=> {
                            s()("#JSroutesSlider").slick({
                                centerMode: !0, arrows: !0, infinite: !0, centerPadding:"0px", slidesToShow:1, fade: !0, cssEase:"ease-in-out", nextArrow:"#JSroutesSlider-next", prevArrow:"#JSroutesSlider-prev"
                            })
                        }

                        , p=()=> {
                            const e=s()(".accordeon"); e.each((function(e, t) {
                                if(s()(t).children().eq(0).hasClass("accordeon__firstChildOpened")) {
                                    s()(t).children().eq(0).addClass("accordeon__item_opened"); let e=s()(t).children().eq(0).find(".accordeon__content").outerHeight( !0); s()(t).children().eq(0).find(".accordeon__contentWrapper").css("height", e+"px")
                                }

                                s()(t).children().each((function(e, n) {
                                    let o=s()(n).find(".accordeon__activator")[0]; s()(o).click((function() {
                                        let e=s()(n).find(".accordeon__contentWrapper")[0], o=s()(n).find(".accordeon__content")[0], a=s()(o).outerHeight( !0); ((e, t, n, o)=> {
                                            s()(t).children().each((function(e, t) {
                                                s()(t).removeClass("accordeon__item_opened"), s()(t).find(".accordeon__contentWrapper").css("height", "0px")
                                            })), s()(e).addClass("accordeon__item_opened"), s()(n).css("height", o+"px")
                                        })(n, t, e, a)
                                    }))
                                }))
                            }))
                        }

                        , _=n(1), f=()=> {
                            const e=s()(".routeSelect")[0], t=s()(".routeSelect__overlay")[0], n=s()(".routeSelect__control")[0], o=s()(".routeSelect__label")[0], a=s()(".routeSelect__subLabel")[0], r=s()(".routeSelect__list-item"), i=s()("[data-routeSelect-info]")[0], c=s()("[data-routeSelect-time]")[0], f=s()("[data-routeSelect-date]")[0], h=s()("[data-routeSelect-price]")[0], m=s()("[data-routeSelect-text]")[0], v=s()("[data-routeSelect-tabs]")[0], b=(s()("[data-routeSelect-tab]"), s()(".routeSelect__select")); let g=()=>s()(e).toggleClass("routeSelect_opened"), y=e=> {
                                    let t="<li><span>Сложность:</span><span>"; for(let n=1; n<=10; n++)n<=routesBD[e].hardness?t+='\n          <svg>\n            <use xlink:href="./assets/icons/sprite-mono.svg#star__fill">\n          </svg>\n        ':t+='\n          <svg>\n            <use xlink:href="./assets/icons/sprite-mono.svg#star__solid">\n          </svg>\n        '; t+="</span></li>", s()(i).append(s()(t))
                                }

                                , S=e=> {
                                    routesBD[e].info.forEach(e=> {
                                        let t=`<li>\n <span>${e.label}:</span>\n <span>${e.text}</span>\n </li>`; s()(i).append(s()(t))
                                    })
                                }

                                , k=e=> {
                                    let t="<li>"; routesBD[e].time.day&&(t+=`\n <span>\n <svg>\n <use xlink:href="./assets/icons/sprite-mono.svg#sun" >\n </svg>\n ${routesBD[e].time.day}\n </span>`), routesBD[e].time.night&&(t+=`\n <span>\n <svg>\n <use xlink:href="./assets/icons/sprite-mono.svg#moon" >\n </svg>\n ${routesBD[e].time.night}\n </span>`), t+="</li>", s()(c).append(s()(t))
                                }

                                , w=e=> {
                                    s()(f).html(routesBD[e].date)
                                }

                                , x=e=> {
                                    let t=`\n <span>${routesBD[e].price.new} ₽</span>\n <span>${routesBD[e].price.old} ₽</span>\n `; s()(h).html(s()(t))
                                }

                                , L=e=> {
                                    s()(m).html(routesBD[e].text)
                                }

                                , C=e=> {
                                    s()(v).html(""); let t=s()(`[data-routeSelect-tab="${e}"]`).html(); s()(v).append(t), l(), u(), d(), Object(_.a)(), p()
                                }

                            ; s()(n).click((function() {
                                g()

                            })), s()(t).click((function() {
                                s()(e).removeClass("routeSelect_opened")

                            })); let O=e=> {
                                    s()(o).html(routesBD[e].name), s()(a).html(routesBD[e].area), s()(b).val(e), s()(i).html(""), s()(c).html(""), y(e), S(e), k(e), w(e), x(e), L(e), C(e)
                                }

                            ; r.each((function(e, t) {
                                s()(t).click((function() {
                                    let e=s()(this).data("routeselectId"); O(e), g()
                                }))

                            })), s()(b).change((function() {
                                O(s()(this).val())
                            })); let q=s()(b).val(); y(q), S(q), k(q), w(q), x(q), L(q), C(q)
                        }

                        , h=()=> {
                            s()("a.scroll-to").on("click", (function(e) {
                                e.preventDefault(); var t=s()(this).attr("href"); s()("html, body").stop().animate({
                                        scrollTop:s()(t).offset().top-150
                                    }

                                    , 800)
                            }))
                        }

                        , m=()=> {
                            s()(".popupOpen").click((function() {
                                return s()(".popup").fadeIn(),  !1

                            })), s()(".popup__close").click((function() {
                                return s()(this).parents(".popup").fadeOut(),  !1

                            })), s()(document).keydown((function(e) {
                                27===e.keyCode&&(e.stopPropagation(), s()(".popup").fadeOut())

                            })), s()(".popup").click((function(e) {
                                0==s()(e.target).closest(".popup__container").length&&s()(this).fadeOut()

                            })), s()(".popupOpenVideo").click((function() {
                                return s()(".popupVideo").fadeIn(), s()(".popupVideo__content").html('\n            <video id="video" preload="none" controls="controls"  autoplay>\n                    <source src="./assets/images/Klyazma-promo.mp4" type=\'video/mp4; codecs="avc1.42E01E, mp4a.40.2"\'>\n                </video>\n                '), document.getElementById("video").play(),  !1

                            })), s()(".popupVideo__close").click((function() {
                                return s()(this).parents(".popupVideo").fadeOut(), document.getElementById("video").pause(),  !1

                            })), s()(document).keydown((function(e) {
                                27===e.keyCode&&(e.stopPropagation(), s()(".popupVideo").fadeOut(), document.getElementById("video").pause())

                            })), s()(".popupVideo").click((function(e) {
                                0==s()(e.target).closest(".popupVideo__container").length&&(s()(this).fadeOut(), document.getElementById("video").pause())
                            }))
                        }

                        , v=()=> {
                            s()(".form").on("submit", (function(e) {
                                let t=this; return s.a.ajax({
                                    url:"/templates/scripts/send.php", method:"post", dataType:"json", data:s()(this).serialize(), success:function(e) {
                                        console.log(s()(t)), s()(t).html('<div class="form__thanks">Ваша заявка принята. Наш специалист свяжется с Вами в ближайшее время.</div>')
                                    }
                                }), e.preventDefault,  !1
                            }))
                        }

                    ; s()(document).ready((function() {
                        let e; i(), c("JSofferSlider"),c("JSofferSlider2"), c("JSofferSlider3"),c("JSofferSlider4"), d(), l(), u(), (()=> {
                            let e=(e, t, n)=> {
                                    t.offsetHeight>n?(e.classList.add("menuLine__btn-wrapper_visible"), t.classList.add("menuLine__head_left")):(e.classList.remove("menuLine__btn-wrapper_visible"), t.classList.remove("menuLine__head_left"))
                                }

                            ; document.querySelectorAll(".menuLine").forEach((function(t, n) {
                                let o=t.querySelector(".menuLine__head"), s=t.querySelector(".menuLine__nav"), a=t.querySelector(".menuLine__btn-wrapper"), r=t.querySelector(".menuLine__btn"), i=t.querySelector(".menuLine__back"), c=o.offsetHeight; t.querySelectorAll(".menuLine__nav-item").forEach((function(e, t, n) {
                                    e.onclick=()=> {
                                        setTimeout((function() {
                                            r.classList.remove("menuLine__btn_active"), i.classList.remove("menuLine__back_active"), o.style.height=c+"px"
                                        }), 300)
                                    }

                                })), window.addEventListener("resize", (function(t) {
                                    e(a, s, c), r.classList.contains("menuLine__btn_active")&&(o.style.height=s.offsetHeight+"px")

                                })), a.onclick=()=> {
                                    r.classList.contains("menuLine__btn_active")?(r.classList.remove("menuLine__btn_active"), i.classList.remove("menuLine__back_active"), o.style.height=c+"px"):(r.classList.add("menuLine__btn_active"), i.classList.add("menuLine__back_active"), o.style.height=s.offsetHeight+"px")
                                }

                                    , e(a, s, c)
                            }))

                        })(), f(), p(), s()("#JSreviewsSlider").slick({
                            centerMode: !0, arrows: !0, infinite: !0, centerPadding:"120px", slidesToShow:3, variableWidth: !0, nextArrow:"#JSreviewsSlider-next", prevArrow:"#JSreviewsSlider-prev"

                        }), h(), m(), v(), s()("#gallerySlider").slick({
                            infinite: !0, slidesToShow:1, slidesToScroll:1, dots: !0, arrows: !0, nextArrow:"#gallerySliderBtnNext", prevArrow:"#gallerySliderBtnPrev"

                        }), r()({
                            mask:"+7 (999) 999-9999"

                        }).mask(document.querySelectorAll(".phoneMask")), s()("#routesOpen").click((function() {
                            s()(".routesContainer").addClass("routes__container_active"), e=s()(".routesContainer").find(".routesClose")[0], s()(e).click((function() {
                                s()(".routesContainer").removeClass("routes__container_active")
                            })), u(), s()("#JSroutesSlider").slick("refresh")

                        })), s()(".burgerMenu__burger, .burgerMenu__headIconWrap, .layerBlack").click((function() {
                            s()(".burgerMenu__burger, .burgerMenu__nav").toggleClass("open-menu"), s()("body").toggleClass("fixed-page"), s()(".container").toggleClass("fixed-page"), s()(".layerBlack").toggleClass("layerBlack_visible")
                        }))
                    }))
                }
            });

        </script>

        <script>
    $("#mobMenuTog").click(function() {
        $("#mobMenu").toggleClass("d-none");
        $('body').toggleClass("overfowHidden");
    });
</script>
<script>
    function showMenuContent() {
        $("#mobMenu").toggleClass("d-none");
        $('body').toggleClass("overfowHidden");
    }
</script>


 <script>
     $('.carousel').carousel();

 </script>

<script>
    $(document).on('ready', function() {
        $(".coachs-slider").slick({
            dots: false,
            infinite: true,
            arrows: true,
            speed: 300,
            slidesToShow: 4,
            slidesToScroll: 1
        });




    });
</script>
<script>
    $(document).on('ready', function() {
        $(".mobile-coachs-slider").slick({
            dots: true,
            infinite: true,
            arrows: false,
            speed: 300,
            slidesToShow: 1,
            slidesToScroll: 1
        });
    });
</script>

<script>
    $(document).on('ready', function() {
        $(".apprentice-slider").slick({
            dots: false,
            infinite: true,
            arrows: true,
            speed: 300,
            slidesToShow: 4,
            slidesToScroll: 1
        });
    });
</script>

<script>
    $(document).on('ready', function() {
        $(".mobile-apprentice-slider").slick({
            dots: true,
            infinite: true,
            arrows: false,
            speed: 300,
            slidesToShow: 1,
            slidesToScroll: 1
        });
    });
</script>

<script>
    $(document).on('ready', function() {
        $(".review-slider").slick({
            dots: false,
            infinite: true,
            arrows: true,
            speed: 300,
            slidesToShow: 1,
            slidesToScroll: 1
        });
    });
</script>

<script>
    $(document).on('ready', function() {
        $(".mobile-review-slider").slick({
            dots: true,
            infinite: true,
            arrows: false,
            speed: 600,
            slidesToShow: 1,
            slidesToScroll: 1
        });
    });
</script>

<script>
    $(document).on('ready', function() {
        $(".mobile-advantage-slider").slick({
            dots: true,
            infinite: true,
            arrows: false,
            speed: 300,
            slidesToShow: 1,
            slidesToScroll: 1
        });
    });
</script>

<script type="text/javascript">
    (function ($) {

        "use strict";

        tippy('.tippy-btn');
        tippy('#myElement', {
            html: document.querySelector('#feature__html'), // DIRECT ELEMENT option
            arrow: true,
            animation: 'fade'
        });

    })(jQuery);
</script>

<!-- The core Firebase JS SDK is always required and must be listed first -->
<script src="https://www.gstatic.com/firebasejs/8.2.1/firebase-app.js"></script>

<!-- TODO: Add SDKs for Firebase products that you want to use
     https://firebase.google.com/docs/web/setup#available-libraries -->
<script src="https://www.gstatic.com/firebasejs/8.2.1/firebase-analytics.js"></script>

<script>
    // Your web app's Firebase configuration
    // For Firebase JS SDK v7.20.0 and later, measurementId is optional
    var firebaseConfig = {
        apiKey: "AIzaSyDqbTrroBnh14P0eIH09Kxe7zCEyg9U_bM",
        authDomain: "school-ball-5b9e2.firebaseapp.com",
        databaseURL: "https://school-ball-5b9e2.firebaseio.com",
        projectId: "school-ball-5b9e2",
        storageBucket: "school-ball-5b9e2.appspot.com",
        messagingSenderId: "981110956853",
        appId: "1:981110956853:web:035680007ea60251f1a27c",
        measurementId: "G-HYEHT80WWC"
    };
    // Initialize Firebase
    firebase.initializeApp(firebaseConfig);
    firebase.analytics();
</script>
<script type="text/javascript">
    jQuery(document).ready(function(){
        $('.phone').mask('+7 (000) 000 00 00', {placeholder: "+ (__) _  __"}).on('input', function() {
            var inputValue = this.value;
            var positionEight = inputValue.indexOf('8');
            // Проверяем, что восьмерка стоит сразу после '+7 ('
            if (positionEight === 4) {
                // Заменяем значение поля ввода, заменяя 8 на 9
                this.value = inputValue.slice(0, positionEight) + '9' + inputValue.slice(positionEight + 1);
            }
        });
    });
</script>


<script>
    function showCoaches() {
        $("#mobMenu").toggleClass("d-none");
        $('body').toggleClass("overfowHidden");
        document.location.href = "https://schoolball.ru/#coaches";
    }

    $(function() {
        $('.jqueryOptions').hide();

        $('#choose').change(function () {
            $('.jqueryOptions').slideUp();
            $('.jqueryOptions').removeClass('current-opt');
            $("." + $(this).val()).slideDown();
            $("." + $(this).val()).addClass('current-opt');
        });
    });
</script>


<script src="https://cdn.datatables.net/1.11.1/js/jquery.dataTables.min.js"></script>




</body>
</html>
