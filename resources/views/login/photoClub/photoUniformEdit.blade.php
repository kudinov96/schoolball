                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                <h4>Информация</h4>
                                <hr>
                                <h4 class="mt-0 header-title">Редактирование блока фото</h4>
{{ $mainindex->images  }}
                                <div id="sortable" class="image-container">
                                    @if (!empty($mainindex->images))
                                        @foreach(explode('|', $mainindex->images) as $index => $row)
                                            <div class="image-item" data-index="{{ $index }}">
                                                <img src="{{ asset('storage/'.str_replace("public/", "", $row)) }}" alt="" style="width: 360px;">
                                                <br>
                                                <form method="post" action="{{ route('deletePhotoUniform', [$mainindex->id]) }}">
                                                    @csrf
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <hr>
                <form id="sendform" method="POST" enctype="multipart/form-data" action="{{ route('updatePhotoUniform', [$mainindex->id]) }}">
                    @csrf
