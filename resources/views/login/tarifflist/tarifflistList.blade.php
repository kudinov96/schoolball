...
@section('content')
    ...
    <div class="align-item-center mt-2">
        <button class="btn btn-dark waves-effect waves-light" onclick="document.location.href = '/tarifflistNew';">Создать новый тариф</button>
    </div>
    ...
    <table class="table">
        ...
        @foreach ($crmtable as $tariff)

    <tr>
        <th scope="row"><a href="/tarifflist/{{ $tariff->id }}/edit">{{ $tariff->id }}</a></th>
        <td><a href="/tarifflist/{{ $tariff->id }}/edit">
                @foreach ($clublist as $club)
                @if($club->id == $tariff->club_id)
                {{ $club->name }}
                    @endif
                @endforeach
        ...
        <td>
            @foreach ($groupage as $age)
                @if($age->group_age_id == $tariff->group_id)
                    {{ $age->name }}
                @endif
            @endforeach
