...
$(function() {
    ...
    @foreach ($crmtable as $client)
        {
            "id": {{ $client->id_crm_camp }},
            "Date": function () {
              var date = new Date("{{ $client->date_create }}");
              return ("0" + date.getDate()).slice(-2) + "." + ("0" + (date.getMonth() + 1)).slice(-2) + "." + date.getFullYear();
            },
            "EntryPoint": {{ $client->entry_point }},
            "Client": "<div id='poster-{{ $client->id_crm_camp }}' class='poster'><button id='client-button-{{ $client->id_crm_camp }}' type='button' class='btn btn-secondary waves-effect mo-mb-2' data-toggle='modal' data-animation='bounce' data-target='#client-form' onclick='clientdataload({{ $client->id_crm_camp }})'> {{ $client->user_name }} </button><div class='descr' id='poster-info-{{ $client->id_crm_camp }}'><div class='row'><div class='col-12'><h4 class='text-light text-center' style='margin: auto;' id='client-name-{{ $client->id_crm_camp }}'>{{ $client->user_name }} </h4><hr><p id='client-phone-{{ $client->id_crm_camp }}'>Телефон: {{ $client->user_phone }}</p></div></div></div></div>",
            ...
            @endif
            "Student": "<div id='poster-kid-{{ $client->id_crm_camp }}' class='poster'><button id='kid-button-{{ $client->id_crm_camp }}' type='button' class='btn btn-secondary waves-effect mo-mb-2'  data-toggle='modal' data-animation='bounce' data-target='#kid-form' onclick='kiddataload({{ $client->id_crm_camp }})'> {{ $client->kid_name }} </button><div class='descr' id='poster-kid-info-{{ $client->id_crm_camp }}'><div class='row'><div class='col-12'><h4 class='text-light text-center' style='margin: auto;' id='kid-name-{{ $client->id_crm_camp }}'>{{ $client->kid_name}}</h4><hr><p id='kid-datebirth-{{ $client->id_crm_camp }}'>Возраст: {{ $age }}@if($age <= 4) года @else лет @endif@php $d = date('d.m.Y', strtotime($client->kid_datebirth)) @endphp ({{ $d }})</p><p>Возрастная группа: {{ $group }}</p></div></div></div></div>",
...
<script>
  ...
  @foreach ($crmtable as $client)
    document.getElementById('poster-{{ $client->id_crm_camp }}').addEventListener('mousemove', function(e){
      document.getElementById('poster-info-{{ $client->id_crm_camp }}').style.top = $('#poster-{{ $client->id_crm_camp }}').offset().top-350+'px';
      document.getElementById('poster-info-{{ $client->id_crm_camp }}').style.left = e.clientX-350+'px';
    });
    document.getElementById('poster-kid-{{ $client->id_crm_camp }}').addEventListener('mousemove', function(e){
      document.getElementById('poster-kid-info-{{ $client->id_crm_camp }}').style.top = $('#poster-kid-{{ $client->id_crm_camp }}').offset().top-350+'px';
      document.getElementById('poster-kid-info-{{ $client->id_crm_camp }}').style.left = e.clientX-350+'px';
    });
  @endforeach
